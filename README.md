...
- **ingest** — Read a paper or article and turn it into wiki pages: one page that summarizes the source and points to its exact evidence, plus short pages for each reusable idea it introduces. It reads the whole source, shows you its plan, and asks before writing anything. After writing, it checks every non-obvious bullet through two separate lenses — whether the locator points to the right evidence, and whether that evidence really supports the wording — and saves a reusable verdict for every bullet. Run it again on the same source later to refresh, deepen, or re-angle the notes. *Use it whenever you add a source, or to update one you've ingested before. This is where everything starts.*
...
- **lint** — A fast, automatic check of the wiki's *structure* — missing sections, broken links, miscounted sources, stray formatting — that quietly fixes the safe, mechanical problems. It also checks every occurrence of a possible unlinked page mention instead of treating a line-level total as the size of the backlog. Cheap, so run it often. *Use it right after an ingest, or any time for a quick structural pass.*
- **audit** — A slower, deeper read that judges whether the notes are actually *good*. It verifies every in-scope bullet through the same locator and entailment lenses used by ingest, then gives each page two fresh whole-page reads for atomicity, contradictions, internal consistency, and argument flow. A normal partial audit can reuse exact, still-valid bullet evidence and rerun only missing or invalid bullet checks; a full audit reruns every bullet from scratch. It records a terminal verdict for every claim and page before promoting anything to "verified" or flagging it for repair. *Use it before you rely on the wiki for something important, or after a batch of ingests.*
...
- **skill-llm-council** — Put one skill in front of two independent councils — one using general thinking lenses, one using skill-specialist lenses — and save the full, anonymous audit trail. It has two deliberately separate modes: `evaluate` assesses an existing skill and writes a report only; `solve` starts from named problems, generates competing implementation-complete solutions, sends the frozen candidates to fresh evaluators, and applies only in-folder changes that survive coherence checks plus independent locator and entailment refuters. Cross-file changes remain proposals. The slower, deeper companion to `skill-linter`. *Use `evaluate` for a second opinion, or `solve` when you want competing ideas for specific skill problems and a guarded implementation pass.*
...
- **Pages have a lifecycle.** A new page is a *draft*. `audit` promotes it to *verified* after re-reading it against its source. Any page can be flagged *needs-update* when something goes stale or a contradiction turns up — and it has to say why. Verification is claim-level without losing page-level review: replacing a page wholesale sends it back to *draft*, while a small addition can mark only that claim as unverified. A partial audit may reuse exact valid verdicts for unchanged bullets, but every page in scope still receives fresh whole-page consistency and argument checks before its status changes.
- **Every ingest is checked bullet by bullet.** Each non-obvious bullet gets a locator verdict and a separate entailment verdict, stored in the ingest report so a later partial audit can reuse it when the bullet, source, locator, and context are still identical. Ingest never awards `verified` status to newly authored content; audit owns the independent page-level gate. When ingest adds a marked claim to an already-verified page, the existing page can keep its status while that claim remains visibly pending.
- **Two kinds of health check.** `lint` is the cheap, automatic one: it checks structure — sections present, links valid, counts correct, formatting clean — and fixes the safe problems itself. `audit` is the slower, thoughtful one: it reconciles a verdict for every claim and adds two fresh page-level reads for source location, atomicity, contradictions, internal consistency, and argument flow.
