...
- **ingest** — Read a paper, book, or article and turn it into wiki pages: one page that summarizes the source and points to its exact evidence, plus short pages for each reusable idea it introduces. It reads the whole source, shows you its plan, and asks before writing anything — for a book it asks first which chapters you want, so it never has to read the whole thing. Before it finishes, it checks every claim it just wrote back against the source. Run it again on the same source later to refresh, deepen, or re-angle the notes. *Use it whenever you add a source, or to update one you've ingested before. This is where everything starts.*
...
- **audit** — A slower, deeper read that judges whether the notes are actually *good* — the questions you can only ask with the whole page in front of you: is this one idea or three that should be split, does it duplicate a page you already wrote, does it contradict something, is it well connected, does it read like an LLM wrote it. Individual claims were already checked against the source when they were written, so audit re-opens a source only when it has a reason to. *Use it before you rely on the wiki for something important, or after a batch of ingests.*
...
- **Pages have a lifecycle.** *Verified* means every claim on the page was checked against its source and held — usually by `ingest` itself, since it has the source open as it writes. A page stays a *draft* when something on it could not be checked (most often a claim citing a source that run did not open). Any page can be flagged *needs-update* when something goes stale or a contradiction turns up — and it has to say why. Verification is claim-level: replacing a page wholesale sends it back to *draft*, but a small edit (say, adding one citation) just marks that one claim as unverified so the rest of the page stays trusted and only that claim gets re-checked.
- **Every claim is checked where it is written.** After writing, ingest asks three narrow questions of each claim it just wrote — is the page number right, does the source really say this, is the claim attached to the source that states it — and puts each one to independent checkers that read the source cold and try to prove it wrong. A claim that survives is marked verified; one that cannot be settled is left visibly flagged rather than quietly accepted.
- **Two kinds of health check.** `lint` is the cheap, automatic one: it checks structure — sections present, links valid, counts correct, formatting clean — and fixes the safe problems itself. `audit` is the slower, thoughtful one: it reads each note as a whole and judges whether it is one clear idea, whether it duplicates or contradicts something else in the wiki, and whether it reads like a person wrote it.
