...
8. **Apply approved changes under ingest discipline.** Approving a page turns this query into a page-authoring pass — a full raw re-read plus the shared claim check and page self-check, heavier than the pure-output common case. A new or substantively edited concept/entity/synthesis page is new durable wiki content, so author it as such: build the page to its `CLAUDE.md` template (the concept/entity or synthesis frontmatter and required callouts), fully re-read the supporting raw source(s) — the original papers, not just the wiki source pages — then run the shared claim check and page self-check (distinct from this skill's Step 6 query packets) as specified in the shared `.claude/skills/multi-skill/references/verification.md` over every touched page and fix what they find before finalizing. The page work is complete only when the claim check comes back clean in the same round — it passes with no fix applied — not merely after one corrective pass each. Set the page's status by that spec's Setting Status rule; a query-promoted page normally lands `draft`, because its claims usually span raws this run did not each re-read, and any claim it could not certify is marked `*[unverified]*` rather than left silently unchecked. Record both check results — the late-section detail, the `#page=N` spot-check, the claim tally, and any fixes — as a short `Promotion verification` section appended to the query output saved in Step 5; do not save a separate report file, and never one under `2-outputs/ingest/` (the shared `verification.md` routes a query's verification into its own query output — one report per query, in the calling skill's own folder). Then do the mechanical bookkeeping: the new page's `sources:`/`source_count`/`Sources`, the supporting source pages' `Concepts and Entities` callouts, every touched page's `updated:`, and `index.md` — and, when the approved page is a synthesis, the synthesis-only frontmatter the ingest-family `verification.md` does not cover: `origin:` (a pipe-rendered wikilink to this saved query output), `single_source_stub: true` only if a single-source stub was explicitly approved (Step 7), and the two-source minimum (CLAUDE.md → Synthesis pages; lint flags a synthesis missing the required `origin:`). (A pure-output query with no durable page — the common case — skips these Step 8 checks; the Step 6 verification still ran.)
