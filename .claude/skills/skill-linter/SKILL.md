...
- [ ] Step 2: Run deterministic checks (check_structure.py, check_synonyms.py, check_musts.py, check_h2_case.py, check_kwargs.py, check_internal_refs.py)
...
2. **Run deterministic checks.**
   ...
   **Capture the exit code and stderr on every invocation** — e.g. append `; echo "EXIT=$?"`. A run succeeded only when it exits 0 AND prints a JSON array to stdout. Any non-zero exit, or empty/non-JSON stdout, means the scanner did not run (a bad path, a missing SKILL.md, a layout mismatch when the linter was copied elsewhere) — surface the stderr text as a hard error and do not count that check as clean. An exit-2 empty stdout is a failed run, not a clean `[]`. These six scanners exist because judgement-only review reliably misses mechanical drift — terminology slips, bare imperatives, reference-file headings, call sites scattered across scripts, cross-references orphaned by a renumbering — once richer findings compete for attention; run all six every pass.
   ...
   Then run the internal-cross-reference scanner:

   ```bash
   python3 .claude/skills/multi-skill/scripts/check_internal_refs.py <skill-dir-or-SKILL.md>
   ```

   This flags a `Step N` or sub-step `N.M` reference whose target is not among the numbered steps or bolded sub-step labels the same file defines — the damage a renumbering leaves behind, where prose keeps pointing at a step that moved or vanished. It uses `check_id: stale_step_reference` and severity `warning`: a dangling step pointer is a factual error in the procedure, not a style preference, and the fix is determinate once you know which step now holds the work. It runs the step check on `SKILL.md` only, since a reference file cites its parent's steps and defines none of its own. It is an existence check and cannot catch a reference that resolves to the *wrong* existing step; that still needs a reader.

   In single-file mode, pass the SKILL.md file path to `check_synonyms.py`, `check_musts.py`, `check_h2_case.py`, and `check_internal_refs.py` (each accepts a file); `check_kwargs.py` returns an empty result for a single SKILL.md since it scans only `scripts/*.py`, so it may be skipped.
...
6. **Iterate full passes until two consecutive runs return zero actionable findings.**
   ...
   2. Re-run the **full skill-linter pass** as one independent run: all six deterministic scripts (the path-qualified `python3 .claude/skills/multi-skill/scripts/check_structure.py`, `check_synonyms.py`, `check_musts.py`, `check_h2_case.py`, `check_kwargs.py`, `check_internal_refs.py` invocations from Step 2 — in single-file mode keep the same single-file argument forms from Step 2, not directory traversal) plus the judgement-based reading from Step 3 and the project rule-set checks from Steps 3-4. A fresh full pass over the whole skill, not an incremental re-check of the lines just edited. A fix can introduce drift in a check it did not touch, and a judgement-level rewrite can re-trigger a deterministic check downstream.
