...
Use whenever the user asks to lint, audit, review, check, validate, critique, or improve the quality of a skill, SKILL.md, skill folder, or `.skill` bundle — even without the word "lint" ("what's wrong with my-skill?", "give me feedback on this skill"). Also use when the user asks whether a skill follows best practices (vague description, too long, nested references, first-person language, time-sensitive info, Windows paths).
...
1. **Load memory, then resolve and validate the input path.**
   ...
   - **File** ending in `SKILL.md` → lint just that file: pass `--single-file` to `check_structure.py` (Step 2), give the other scanners the bare `SKILL.md` path with no flag, and skip `check_kwargs.py`. Bundled-file checks (reference traversal, `check_kwargs.py`) are skipped; note this in the report header.
...
2. **Run deterministic checks.**
   ...
   **Capture the exit code and stderr on every invocation** — e.g. append `; echo "EXIT=$?"`. A run succeeded only when it exits 0 AND prints a JSON array to stdout. Any non-zero exit, or empty/non-JSON stdout, means the scanner did not run (a bad path, a missing SKILL.md, a layout mismatch when the linter was copied elsewhere) — surface the stderr text as a hard error and do not count that check as clean. An exit-2 empty stdout is a failed run, not a clean `[]`. These five scanners exist because judgement-only review reliably misses mechanical drift — terminology slips, bare imperatives, reference-file headings, call sites scattered across scripts — once richer findings compete for attention; run all five every pass.
   ...
   python3 .claude/skills/skill-linter/scripts/check_structure.py <skill-path>

   # Single SKILL.md file (skips reference traversal)
   python3 .claude/skills/skill-linter/scripts/check_structure.py <path-to-SKILL.md> --single-file
   ```

   Pass `--single-file` only when the user explicitly pointed at a single SKILL.md file rather than a directory; without the flag the script also examines any `references/*.md` file that SKILL.md links as a Markdown hyperlink (a bracketed label with a parenthesized `.md` target), rather than as an inline-code path. Its reference-structural checks (`broken_md_link`, `nested_reference`, `missing_toc`) key on that hyperlink syntax, so a skill that cites its references only as inline-code paths gets no reference-structural coverage from `check_structure.py` — `check_h2_case.py` is the one scanner that globs every `references/*.md` sibling directly. `check_structure.py` produces a JSON list of structural findings. Each finding has `severity`, `check_id`, `file`, `line`, `message`, and `fix_hint`. See `references/checks.md` for the complete list of what this script checks and the severity each check uses.

   Then run the synonym-candidate scanner:

   ```bash
   python3 .claude/skills/skill-linter/scripts/check_synonyms.py <skill-dir-or-SKILL.md>
   ```
   ...
   - **Intentional distinction** (the terms refer to different concepts) → drop the candidate. The skill is fine; the script was being cautious.

   Then run the heavy-handed-imperative scanner:

   ```bash
   python3 .claude/skills/skill-linter/scripts/check_musts.py <skill-dir-or-SKILL.md>
   ```

   This script flags ALL-CAPS directives (`ALWAYS` / `NEVER` / `MUST` / `DO NOT`) in body paragraphs that don't contain a nearby explanation cue (e.g. `because`, `to avoid`; the full cue set is in `references/checks.md`). The check uses `check_id: heavy_handed_must_candidate` for the same reason as the synonym scanner: a script can detect *that* the imperative is bare, but you have to decide *whether* the missing reasoning matters. For each candidate:
...
3. **Read for judgement-based checks.**
   ...
   Some best-practices issues require reading the skill, not pattern-matching. Open `SKILL.md` (and any `references/*.md` files referenced from it) and apply the rubric in `references/checklist.md`. That file lists each judgement check, what to look for, what to flag, what severity to assign, and how to phrase the fix suggestion.
...
6. **Iterate full passes until two consecutive runs return zero actionable findings.**
   ...
   Self-lint caveat: when the target is skill-linter itself, an in-folder fix to one of its own `scripts/*.py` (the deterministic scanner) or `references/*.md` (the judgement rubric) changes the scanner the loop re-runs; the two-clean-pass counter — which the loop already resets on any applied fix — therefore measures convergence against the post-edit scanner. Before trusting a modified scanner's output, run it against an input you know it should flag (a deliberately-broken fixture) and confirm it still flags — an edit that makes a scanner wrongly emit `[]` exit 0 on bad input converges falsely, and the exit-2 guard cannot catch a scanner that incorrectly exits 0.
...
For the full list with severities and rationales, see `references/checks.md`. At a glance: missing/invalid frontmatter, name and description schema violations (length, casing, reserved words, third-person), `SKILL.md` body over 500 lines (see `scripts/check_structure.py` `SKILL_MD_MAX_LINES`), raw HTML tags in markdown prose, bash process substitution `<(...)` in prose (upload-breaking), Windows-style paths in markdown, references nested more than one level deep from `SKILL.md`, reference files over 100 lines that lack a table of contents (see `scripts/check_structure.py` `REFERENCE_TOC_THRESHOLD`), and obviously-broken markdown link targets that point at files that don't exist in the skill directory. The four separate scanners (`check_synonyms.py`, `check_musts.py`, `check_h2_case.py`, `check_kwargs.py`) are documented there too.
