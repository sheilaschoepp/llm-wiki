...
## A crop is also the later reader's only route into the figure

An extracted crop is not only for display on the page. A figure that is an embedded raster with no text layer is unreadable to `pdftotext`, so a later verification pass — this skill's own claim check, or `audit` — cannot confirm a value the figure prints unless the crop exists and is legible. That makes two things load-bearing beyond appearance: extract the figure a claim draws its numbers from even when the page's prose would read fine without it, and keep the crop legible at full size (printed data labels, axis values, legend percentages readable), because those are exactly the values a refuter will need. A claim whose figure was never extracted is one a later pass may have to mark `*[tentative]*` on a technicality of extraction tooling rather than on any doubt about the claim.
