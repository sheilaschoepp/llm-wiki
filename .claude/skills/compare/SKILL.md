...
Schema note: `target_type` is one of `source | concept | entity | synthesis` (a source target's `target_type` is `source`, not its `paper` / `article` / `book` / `media` sub-type); list 2-4 targets in `targets:` as wikilinks, one table value column per target.
