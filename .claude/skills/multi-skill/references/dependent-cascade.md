...
Verification is claim-level (see `CLAUDE.md` → Page Status). When a cascade adds or changes a non-obvious claim on a `verified` dependent without re-fact-checking that claim against the raw, mark just that claim `*[unverified]*` (Bullet Markers) — `verified_hash` excludes marked claims, so the page stays `verified` with the marked claim as the pending delta, and the next `audit` re-checks only it. A change to *unmarked* (checked) content — an embed, a new bullet, a citation retargeted to a different source, or a `Sources` / `sources:` entry added or removed — does change the hashed body and the page's support: reset the page to `status: draft` and strip its now-stale `verified_hash:` in the same edit (a non-`verified` page carries none; a later `audit` re-adds it on verify). If you do not reset such a page explicitly, `lint` demotes it via `verified_hash` on its next run anyway — set it so the page is honest immediately. Mechanical frontmatter touches (`updated:`, `source_count:`) are hash-safe.
