...
Verification is claim-level for additions (see `CLAUDE.md` → Page Status). When a cascade **adds a new** non-obvious claim bullet to a `verified` dependent without re-fact-checking it against the raw, mark just that claim `*[unverified]*` (Bullet Markers) and the page stays `verified` with the marked claim as the pending delta: `verified_hash` excludes marked claims, and a newly-added masked line was never in the hash, so the hash does not move and the next `audit` re-checks only it. **A single added statement is this case — mark the statement; it is never grounds to reset the whole page.** **Changing an existing claim is not this case:** editing a verified claim's text, or retargeting its citation to a different source or locator, moves the hash whether or not you mark it (the old line was already in the hash), so it resets the page to `status: draft`. Reset to `draft` (stripping the now-stale `verified_hash:` in the same edit; a non-`verified` page carries none, a later `audit` re-adds it on verify) for any change a claim marker cannot carry: a change to an existing (already-checked) claim, an embed added or removed, or a `Sources` / `sources:` entry added or removed — a change or support-level shift rather than a clean addition. If you do not reset such a page explicitly, `lint` demotes it via `verified_hash` on its next run anyway — set it so the page is honest immediately. Mechanical frontmatter touches (`updated:`, `source_count:`) are hash-safe.

**Verification-neutral link maintenance re-stamps instead of demoting** (CLAUDE.md → Page Status, the verification-neutral allowlist). Two link operations change no claim and so keep the page `verified`: wrapping an already-present plain-text reference in a wikilink (the rendered display stays byte-identical and the target page exists — the `unlinked_page_mention` fix), and a stale-path repair that rewrites an inbound link to the *same* page under its new path/name (display unchanged). For these, apply the edit and **re-stamp** `verified_hash:` to the fresh `body_hash.py` output in the same pass, keeping `status: verified` — no fact-check. This carve-out is *only* those two operations. Changing *which* source backs a claim — a `sources:` add or removal, a citation retargeted to a different source page or a different locator — is claim-relevant, not neutral, and (being a change to an existing claim, not an addition) it demotes the page to `draft` as above. When unsure whether a link edit changes a claim, treat it as non-neutral and demote. Note any `verified → draft` demotion in the log entry so the audit trail captures the regression.
