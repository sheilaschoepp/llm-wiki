...
Hard exclusion: *adding* an anchor to a page-only locator, or *changing* which section/figure/page a locator names (e.g. relabelling abstract-drawn content `sec. 1`), is **not** on the allowlist. That asserts a new fact about where the cited content sits, which only the raw can settle. Two routes, never a bare re-stamp: the run opens the raw at that page, settles the anchor by the nearest heading above the cited text, certifies it through the refuter tiers, and stamps on that certification; or it marks the bullet `*[unverified]*` / demotes the page to `draft` for a later run. What stays forbidden is the middle path — inferring an anchor from the page number or from the wiki page and stamping the inference, which is what produced the abstract→`sec. 1` mislabels. The guard is whether the raw was opened at that page, not which skill did the opening. `lint` emits the Warning `verified_anchor_unaudited` when a `verified` page's locator anchor changed versus git HEAD (an addition or relabel; a pure relocation is exempt) — it cannot see whether a run certified it, so audit carries it on its authored worklist and settles any anchor no run recorded certifying.
