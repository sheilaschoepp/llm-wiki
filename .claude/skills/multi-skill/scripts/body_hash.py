#!/usr/bin/env python3
"""
Print the SHA-256 of a wiki page's body — the content after the YAML
frontmatter.

`audit` runs this to stamp `verified_hash:` when it sets a page
`verified`; `lint` runs it to detect a `verified` page whose checked
content changed since (hash mismatch -> the page is reset to `draft`).
Both skills must hash identically, so both call this one script. The
body excludes frontmatter, so mechanical metadata edits (`updated:`,
`status:`, `verified_hash:` itself) do not change the hash.

The hash also excludes any bullet line carrying the `*[unverified]*`
claim marker (CLAUDE.md -> Page Status, Bullet Markers): verification is
claim-level, so a claim still pending a raw fact-check must not count
toward the page's checked content. This is what lets a `verified` page
accept an incremental ADDITION (a new claim bullet, a new citation on
its own bullet) marked `*[unverified]*` without tripping the hash and
demoting the whole page: a masked new line was never in the hash.
Changing an existing claim is different — altering its text, or newly
marking a previously-unmarked (already-hashed) claim, moves the hash and
demotes the page; editing or adding an UNMARKED claim likewise changes
the hash and demotes it (the involuntary backstop). A page with no
markers hashes exactly as it did before this masking existed, so
existing stamps stay valid. Keep a marked claim to its single bullet
line — only the line carrying the marker is masked, so a multi-line
marked bullet's continuation still counts.

The hash function itself is deliberately simple: it masks only
`*[unverified]*` claim lines and frontmatter, nothing else. The
verification-neutral edit allowlist (CLAUDE.md -> Page Status) is
enforced at the skill level, not here: a skill that applies an
allowlisted determinate edit (a format fix, an open-compound de-
hyphenation, a spelling normalization, or wrapping an existing reference
in a wikilink) recomputes this hash and re-stamps `verified_hash:` in
the same pass, keeping the page `verified`. So adding allowlist handling
does not touch this script — it only changes whether a skill re-stamps
or demotes after a hash change.

Requires Python 3.10+ (the shared lint-script suite floor; this module
alone uses only 3.8-compatible syntax, but the suite is pinned to 3.10+
by check_wiki.py).
"""
...
def body_hash(path: str) -> str:
    """
    Return the hex SHA-256 of the page body — the text after the YAML
