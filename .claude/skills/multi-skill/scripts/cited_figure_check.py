...
-> Source Support And Verification, the multi-source-bullet rule): a
true, well-formed claim carrying a distinctive figure is cited to a raw
`#page=N` deep-link whose page does not carry that figure. The claim
reads correctly and passes every structural check in `check_wiki.py`;
only opening the cited page reveals the number is not there.

This lives OUTSIDE `check_wiki.py`'s battery on purpose. `check_wiki.py`
never opens a raw source (lint/SKILL.md Limits: "Lint does not read raw
source content"); this script does, so it is a separate, opt-in tool the
agent runs deliberately, not part of the structural lint pass. It
requires PyMuPDF (`fitz`), installed in the `llm-wiki` conda env by
setup.sh.
...
`warning` (detect-only). Exit code 0 normally; 2 on a usage error (bad
wiki path); 3 if PyMuPDF is missing (so a caller distinguishes "check
could not run" from "found nothing").
"""
...
# A raw-source deep-link: group 1 = repo-relative raw path, group 2 =
# physical page N, group 3 = display text. Widens check_wiki.py's
# CITATION_DEEPLINK_RE to also capture the path and page so the raw can
# be opened at N.
DEEPLINK_RE = re.compile(r'\[\[(0-raw/[^\]|#]+)#page=(\d+)\|([^\]]*)\]\]')

# Inline-code and wikilink spans, blanked out of a bullet before figures
# are read, so a locator (`sec. 3.1`, `p. 4`), a link path/display, or a
# code snippet is never mistaken for a claim figure.
CODE_SPAN_RE = re.compile(r'`[^`]*`')
WIKILINK_RE = re.compile(r'\[\[[^\]]*\]\]')

# A distinctive numeric figure: a percentage (integer or decimal) or a
# bare decimal. The lookbehind for a word char, dot, or hyphen — plus
# the `(?!\.\d)` lookahead on the decimal arm — keeps a version or model
# id (`gpt-3.5`, `v2.0`, `3.1.4`) from reading as a figure, while
# `(?!\.\d)` still admits a sentence-final `37.2.` (a dot NOT followed
# by a digit). A bare integer (no dot, no `%`) is out of scope by
# construction.
    ...
    def _extract(self, *, raw_rel: str, page_n: int) -> str | None:
        ...
        if doc is None:
            ...
            try:
                ...
                # A real open failure (encrypted, corrupt, not really a
                # PDF — fitz.FileDataError / EmptyFileError subclass
                # RuntimeError) is "cannot check", the safe direction —
                # not a claimed absence. A code bug
                # (AttributeError/NameError/…) still propagates rather
                # than masquerading as cannot-check (mirrors the
                # _git_show_head narrowing).
                return None
        ...
        try:
            ...
            # Real page-load/parse failure is "cannot check"; a code bug
            # propagates.
            return None
    ...
    *,
    ...
    for line_no, line in enumerate(
        ...
        if any(text is None for text in texts):
            # A cited page is unreadable — cannot rule out presence
            # there.
            continue
