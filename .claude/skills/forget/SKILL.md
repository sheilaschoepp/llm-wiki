...
Forgetting removes content from the active wiki while preserving a quarantined record when a whole page or attachment is removed.

Recoverability differs by scope. Whole pages and attachments are copied to `2-outputs/quarantined/` before deletion, so they are recoverable from quarantine plus git. In-place edits — a bullet or section removed, a support link dropped, an embed deleted — leave no quarantine artifact: they are recoverable from git history only, and not at all if the change was never committed. The cascade plan tags which is which so the user can weigh each veto honestly.
...
- Obtain timestamps at write time with `TZ='UTC' date '+%Y-%m-%d-%H%M'`; the session context provides the date but not the current minute.
...
3. **Build the cascade plan.** Show:
   ...
   Status changes:
     - Scaled Dot-Product Attention -> needs-update (lost a supporting source — an unmarked change to verified content, so it leaves `verified`; flagged needs-update for the lost support)
   ...
   Quarantine targets:
     - 2-outputs/quarantined/quarantined-2026-05-04-1430-Vaswani2017AttentionIA.md
     - 2-outputs/quarantined/attachments/Vaswani2017AttentionIA/
...
6. **Repair source support, links, and downstream status.**
   - Rewrite or remove every inbound body wikilink found in Step 2 so the wiki has no dangling prose links to the removed page.
...
8. **Update `index.md`, `hot.md`, and `log.md`.** Remove the forgotten page's entry from `index.md` (Sources/Entities/Concepts/Syntheses), and confirm it no longer appears there. Remove or rewrite any `hot.md` Open-threads, Watchlist, or stale Recent-activity bullet that named the target (Active focus is user-owned — surface a mention there rather than auto-rewriting it), and add a Recent-activity line for this forget in the schema's `- [YYYY-MM-DD HH:MM] forget | {target}` form (24-hour UTC, the same timestamp as the log heading).

```markdown
## [YYYY-MM-DD HH:MM] forget | {target} - {reason}
- Quarantined: [[2-outputs/quarantined/quarantined-YYYY-MM-DD-HHMM-{filename}.md|quarantined-YYYY-MM-DD-HHMM-{filename}]] (or "none")
...
Pages quarantine to `2-outputs/quarantined/quarantined-YYYY-MM-DD-HHMM-{filename}.md` (date-prefixed). Attachment files and whole-stem folders move to `2-outputs/quarantined/attachments/{stem}/` with their original names — no date stamp, because the `{stem}` folder is the grouping key. For the name-clash suffix rule (`-N` before the extension; the `.gitignore` trap) and the preserved-copy verification, see `.claude/skills/multi-skill/references/quarantine-path-convention.md`; point the log link at the suffixed name.
