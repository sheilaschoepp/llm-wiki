...
7. **Verify the removal landed cleanly.** Before touching the log, read back every changed page and confirm:
   ...
   - Run `python3 .claude/skills/multi-skill/scripts/check_wiki.py "1-wiki"` on every forget to confirm no embed or `sources:` link is left dangling. Guard the parse: `check_wiki.py` prints its findings JSON only on a run that ran to completion, so **empty or unparseable stdout is not "zero findings" — it is a run that could not complete** (a wrong path exits 2, a crashed check aborts with a traceback on stderr; both print nothing, whereas a genuinely clean wiki prints `[]`). Stop and say so rather than treating silence as a clean result. A crashed run must not read as "nothing dangling" (it does not catch body-prose wikilinks — those are the by-eye check above).
