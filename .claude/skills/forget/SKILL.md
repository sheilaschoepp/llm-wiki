...
- Use `AskUserQuestion` for every user-facing decision, batching as far as the decisions allow (`CLAUDE.md` → Workflow Rules). Deletions of **git-recoverable** copies may go out as one `multiSelect` batch (`CLAUDE.md` → Safety rules), as may a slate of same-type status resets; a removal that is **not** git-recoverable — an uncommitted working-tree file, or the only copy of wiki content — stays strictly one question per file, and a zero-source quarantine call is asked on its own because it needs its own framing. Order the recommended option first and mark it `(Recommended)` (CLAUDE.md → Communication style); on these gates the lean is usually clear (the most-likely scope; `proceed` on a deletion the user invoked forget to make; the editorial default for a zero-source or stub call), so declare `no recommendation` only for a genuine no-lean.
...
3. **Build the cascade plan.** Show:
   ...
   - Any synthesis page dropping to exactly one distinct work (which can happen while `sources:` still lists two — a chapter-split pair) (needs a deliberate stub-or-remove decision — `single_source_stub: true`, re-source, or `forget` — not a blanket `needs-update`); a synthesis dropping to zero sources takes the zero-source decision instead (`single_source_stub: true` is invalid with no sources).
...
5. **Apply approved removal.** This step is the dispatcher; the exact copy, verify, and move command sequences live in `references/removal-mechanics.md`. Run them only after Step 4 approval. Three ordering invariants hold regardless of scope and must not be reordered even if the reference is skipped: copy to quarantine and confirm the copy exists, is byte-identical to the original (`cmp -s` for a file, `diff -rq` for a folder — existence/`test -f` passes on a truncated binary), and is not on a git-ignored name (`git check-ignore -q` returns non-zero) before unlinking the original; for any attachment removal (single file or whole folder), remove embeds from referencing pages before unlinking the underlying file; all `status:` changes happen in Step 6, not here.
   ...
   - Support link: remove the source from `sources:` and the `Sources` callout; recompute `source_count` as the distinct works behind the surviving `sources:` list — chapter splits of one raw count once — never a blind decrement and never the list length. Reconcile any body bullet that attributed a claim to the now-removed source.
