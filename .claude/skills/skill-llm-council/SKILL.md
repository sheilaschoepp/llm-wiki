...
1. **Freeze the mode; load memory; resolve and snapshot the target; build the brief.**
   ...
   In `solve`, freeze a nonempty problem manifest before the launch. Each entry has a stable `P_ID`, the named problem, literal verified ground and target anchor, success test, constraints, dependencies, and an out-of-scope boundary. A problem without verified ground or a success test is not ready for `solve`.
