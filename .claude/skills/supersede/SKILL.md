---
name: supersede
description: Replace an active source page, concept/entity page, synthesis page, or specific supported idea with a corrected version while preserving the prior view. Use when the user says a note is stale, wrong, superseded by newer evidence, or needs a refined interpretation. Also use when the user asks to replace, swap, merge, split, or re-extract an existing wiki page or attachment while keeping a record of the prior version. Different from forget, which removes without replacement; different from ingest existing-source mode, which corrects a source page in place from its raw (git preserves the prior version) — supersede is for swaps, merges, splits, and re-extractions that warrant an explicit preserved prior view under 2-outputs/superseded/.
---
...
Supersede keeps the active wiki current without pretending the older view never existed. The prior view is preserved as a vault-navigable artifact — a `2-outputs/superseded/` file or an in-callout bullet, browsable in Obsidian and linkable from `log.md` — which is why supersede captures it explicitly rather than leaving it to git: git preserves the bytes, but not a view the user can open and link. (A plain in-place correction from the raw, where a git blob suffices, is `ingest` existing-source mode, not supersede.)

Two terms recur below and refer to distinct things: a **prior-view artifact** is the preserved old content (a `Prior view` bullet inside a callout, or a file under `2-outputs/superseded/`), while a **log entry** is the dated reverse-chronological line written to `1-wiki/log.md`.
...
3. **Build the plan.** Post a single chat context message that shows:
   ...
   - The target's git state (`git status --short -- <target>`): if it has uncommitted changes, flag that git holds no recoverable pre-edit baseline, and that a small in-callout or merge-survivor overwrite has no `2-outputs/superseded/` fallback either — so the user commits first or proceeds knowingly (CLAUDE.md Safety rules).
   - Pages whose `sources:`, `Connections`, or `Concepts and Entities` sections change.
   - For attachment replacements: which file is being replaced, where the prior file will be preserved (`2-outputs/superseded/attachments/{stem}/superseded-YYYY-MM-DD-HHMM-{name}.png`), the new extraction parameters, and which other pages embed the file.
...
4. **Ask every decision via `AskUserQuestion`.** One decision per call, never batched; order the recommended option first and mark it `(Recommended)` in its label (CLAUDE.md → Behavioural defaults). Typical questions:
   ...
   5. One question per merge weaker-page removal, naming the deletion explicitly, e.g., "Merge `[[...weaker...]]` into `[[...stronger...]]`, then delete the weaker page from `1-wiki/` (preserved copy kept under `2-outputs/superseded/`)?" Options: merge-and-delete-weaker / keep-separate / other. When both pages are `verified` (a duplicate audit surfaced), there is no automatic "stronger" — put the merge-direction (which page survives) in this question; the merged-in content lands on the surviving page as `*[unverified]*` claims (page stays `verified`) unless re-fact-checked in this pass, per the claim-level model (Step 6, `CLAUDE.md` → Page Status).

   Verification is claim-level on a `verified` page, so status is not a discretionary question (`CLAUDE.md` → Page Status): a supersede that adds or changes a non-obvious claim it does not re-fact-check against the raw in this pass marks that claim `*[unverified]*`, and the page stays `verified` with the marked claim as its pending delta (see Step 6); only an unmarked change to verified content demotes the whole page to `draft`. Either way the outcome is mechanical, so do not offer "preserve current status" as a choice. Confirm in the plan how each `verified` target lands — claims marked `*[unverified]*` (page stays `verified`) or the page demoted to `draft` — and that any demotion will be noted in the log; a page already at `draft` / `needs-update` needs no status question.
...
5. **Apply the supersession.** Two ordering invariants hold regardless of scope: the prior view must be captured — the `Prior view` bullet written from the verbatim old text, or the file copied to `2-outputs/superseded/` (to a non-clashing name: pre-check `test -e <dest>` and apply the `-N` suffix rule before the `cp`, so a same-minute re-run cannot clobber the first prior view) and confirmed byte-identical to the original (`cmp -s` for a file, `diff -rq` for a folder — plain existence / `test -f` passes on a truncated or half-copied binary) AND not on a git-ignored name (`git check-ignore -q <path>` returns non-zero) — before the active content is overwritten, never after. Run the not-git-ignored check here, before the overwrite, not only in Step 7: `test -f` passes on a git-ignored name (a mis-formed `.N`-after-extension suffix matches `.gitignore`'s `*.[1-9]`; the `-N`-before-extension clash rule prevents such names, it is not their source), and a post-overwrite check can only report the lost prior view, not prevent it. The second invariant: all `status:` changes happen in Step 6, not here. Copy attachments with shell `cp` only — never round-trip a binary (`.png`/`.jpg`/`.svg`) through Read/Write, which corrupts it; pages are text and may use Read/Write. The Step 3 plan already surfaced the target's git state (`git status --short -- <target>`); do not overwrite a dirty target the user has not committed or knowingly cleared, since git then holds no recoverable pre-edit baseline (CLAUDE.md Safety rules). If any apply step fails after the prior view is on disk but before the active content is consistent, stop, restore the active target from its `2-outputs/superseded/` copy (`cp` back, or re-apply the verbatim `Prior view` bullet text), reverse any cascade edits already made, and surface the failure — never leave a half-applied overwrite.
   ...
   - Whole-page replacement: copy the old page to `2-outputs/superseded/superseded-YYYY-MM-DD-HHMM-{filename}.md` and confirm the copy exists (`test -f`) before replacing the active page — never overwrite before the prior view is confirmed on disk. If the old page had attachments, also copy the attachments folder with `cp -R` (never Read/Write, which corrupts binaries) to `2-outputs/superseded/attachments/{stem}/superseded-YYYY-MM-DD-HHMM/` and confirm it copied byte-identical with `diff -rq` (a folder-exists / `test -d` check passes on a partial `cp -R`). Obtain the timestamp at write time by running `TZ='UTC' date '+%Y-%m-%d-%H%M'`; the session context provides the date but not the current minute.
   - Source-page replacement splits on the stem: if the replacement re-extracts the same raw (same stem), replace in place as above. If it is a different source with its own bibkey/stem, the source-page filename must match the new raw stem (CLAUDE.md → Page Filenames), so this is a rename cascade — create the new page at `1-wiki/sources/{new-stem}.md` with its `file:` frontmatter pointing at the new raw (`[[0-raw/...]]`), copy the old page to `2-outputs/superseded/` and confirm it exists, re-point every inbound reference found in Step 2 to the new stem — including BOTH halves of every inline citation: the `[[1-wiki/sources/{old-stem}.md|…]]` source-page wikilink AND its paired `[[0-raw/.../{old-stem}…#page=N|…]]` raw deep-link, whose stem also changes. The Step-2 grep is `1-wiki/`-only and `.md`-anchored, so it finds the wikilink half but not the `0-raw/` raw-link half; grep `0-raw/.../{old-stem}` separately and rewrite each by hand (apply the Step 6 link-rewrite mechanics here, not deferred). This neutral stem-swap is correct only for a same-source filename migration (a legacy stem → the raw stem, same source — re-stamp `verified_hash:` on any `verified` inbound page); when the new source is genuinely different, the mechanical swap is unsafe — re-judge each inbound page with `dependent-cascade.md`'s genuine-draw test before re-pointing its `sources:` / citation (source B may not support the claim source A backed), and treat the transplanted `#page=N` and printed-page locators as the OLD paper's coordinates: do not carry them over unchanged, but re-verify each against the new raw or mark the claim `*[unverified]*` rather than ship a plausible-but-wrong deep-link. Verify by re-grepping that no inbound link names the old stem in EITHER half, and only then delete the old source page. Surface the rename as its own `AskUserQuestion` (Conventions recap).
   - Split: copy the current old page to `2-outputs/superseded/superseded-YYYY-MM-DD-HHMM-{filename}.md` and confirm the copy exists before mutating it (a split rewrites the donor page's body), then create the approved new page(s), move only the relevant idea, and leave a connection from the old page. Each split-off page ships schema-complete: full required frontmatter (`type`, `aliases: []` for a concept/entity page, `tags: []`, `sources`, `source_count` matching, `created` today, `updated`, `status: draft`, and `origin:` for a synthesis) and the full required callout set for its page type — each callout carrying its block ID, with an empty placeholder where a section has nothing genuine. A split that creates a new synthesis must satisfy the ≥2-source rule or set `single_source_stub: true`. After the split, confirm the donor page still reads coherently and that any citation which relocated with the moved idea now lands on the new page (no orphaned locator left on the donor).
   - Merge: keep the surviving page (the stronger page — `verified` over `draft`, else the higher `source_count`, else ask; or, when both are `verified`, the user-chosen survivor from Step 4 Q5, since there is no automatic "stronger" between two verified pages); first copy BOTH the weaker duplicate AND the surviving page to `2-outputs/superseded/superseded-YYYY-MM-DD-HHMM-{filename}.md` and confirm each copy byte-identical, then move useful content into the surviving page, then delete the weaker page from `1-wiki/` (the surviving page is overwritten by the moved-in content, so it needs its own preserved snapshot as a rollback source; copy-and-confirm precedes moving content, per the Step 5 invariant — a half-moved page must already be preserved). If the user wants outright removal with no preserved view, hand off to `forget` rather than improvising a quarantine here — supersede keeps every prior view under `2-outputs/superseded/`.
   - Attachment replacement: copy the existing file to `2-outputs/superseded/attachments/{stem}/superseded-YYYY-MM-DD-HHMM-{name}.png` and confirm it is byte-identical to the original (`cmp -s` — a plain existence check passes on a truncated copy, and this binary cannot be eyeballed or recovered once the original is overwritten), then place the new extraction in place at `1-wiki/attachments/{stem}/{name}.png` (same path, so existing embeds keep resolving without edits) with `cp` — never the Write tool, which corrupts binaries. Supersede does not re-render the figure itself: the new binary is supplied by the user or re-extracted by `ingest` (`pdftoppm`) out of band; supersede only swaps it in. Add a one-line note to the source page noting the replacement and reason.
...
6. **Maintain metadata and cascade `needs-update`.**
   ...
   - Touch `updated:`.
   - Status on the edited page (claim-level, `CLAUDE.md` → Page Status): verification is per-claim on a `verified` page, so a supersede that adds or changes a few non-obvious claims — a merge that moves content in, a re-extraction, or a small in-callout bullet rewrite — marks each such claim it does not re-fact-check against the raw in this pass `*[unverified]*` (`CLAUDE.md` → Bullet Markers), and the page keeps `status: verified`: the marked claims excluded from `verified_hash` are the pending delta a later `audit` re-checks. A change that instead rewrites essentially the whole verified body — a whole-page replacement, or a split that restructures the donor — cannot mark claim-by-claim (marking every claim would empty the hashed body), so it demotes the page to `draft` instead (see the whole-page Worked Example). A claim the supersede does re-fact-check against the raw this pass needs no marker. Demotion to `draft` happens only when an unmarked change reaches verified (hashed) content — then reset to `draft`, strip the stale `verified_hash:`, and note the demotion in the log, per the shared `verified_hash` mechanics in `.claude/skills/multi-skill/references/dependent-cascade.md`. A page already at `draft`/`needs-update` carries no marker (its status already means unverified) and no hash; leave its status as is. (The cascade reference's blanket "any body change resets to draft" applies to dependents the supersede does not mark; the edited page is marked here, so its delta rides in `*[unverified]*` claims.)
   - On every page that cited the superseded source, apply the shared source-support bookkeeping and under-supported-page rules in `.claude/skills/multi-skill/references/dependent-cascade.md` — `sources:` / `source_count` / `Sources` sync, the zero-source decision (a page left citing no source — quarantine, re-source, or needs-update; including an author entity left with no surviving source), and the single-source-synthesis decision (a synthesis dropping below two sources — stub, re-source, or forget); when re-sourcing, add the replacement only if the page genuinely draws on it. A merge survivor that *gains* the weaker page's source(s) syncs its own `sources:` / `source_count` / `Sources` callout to match — the bookkeeping above covers pages that cited the *superseded* source, but a merge survivor is gaining one, not losing one.
...
7. **Verify the supersession landed cleanly.** Before touching the log, run content verification over what this operation wrote, then confirm the structural landed-cleanly checks:
   ...
   - The active page reads as intended; the prior-view artifact is in the place you said it would be (`Prior view` bullet, or `2-outputs/superseded/...` file).
   - Inbound wikilinks that pointed at the old page or claim now land on the active surviving page, not the `2-outputs/superseded/` copy. Where a rename, split, or merge changed a filename, confirm each inbound link was rewritten to the surviving target — any link still pointing at the removed/renamed name is a failure to fix here, not to defer.
...
8. **Update index, hot, and log.** Update `index.md` for any created, removed, or renamed page; rewrite or remove any `hot.md` Open-threads / Watchlist bullet (found in Step 2) that named the target, and add a Recent-activity line in the `- [YYYY-MM-DD HH:MM] supersede | {target}` form (24-hour UTC, the same timestamp as the log heading). Then prepend the log entry:
...
Apply: edit the `Idea` callout, append the prior-view note, touch `updated:`. Status: this example targets a draft page, so it stays draft (no `*[unverified]*` marker — `draft` already means unverified); had the page been `verified` and the corrected claim not re-fact-checked against the raw this pass, mark the rewritten bullet `*[unverified]*` and the page stays `verified` (Step 6, claim-level model) — no demotion, no log note for status. Log entry: `supersede | multi-head-attention - corrected per-step → per-message claim`.
...
Prior view: copy current page to 2-outputs/superseded/superseded-{YYYY-MM-DD-HHMM}-positional-encoding.md
Cascade:
...
- Botched or vetoed supersession: the prior view captured in Step 5 is the rollback source. If the user vetoes a remaining cascade decision after the active content was already overwritten, or the new content proves wrong, restore the active page from its `2-outputs/superseded/` copy (or re-apply the verbatim `Prior view` bullet text for a small replacement), reverse any cascade edits already made, and abort — never silently re-supersede.
- Target is itself a preserved prior view: a file under `2-outputs/superseded/` is frozen audit trail (same guarantee as historical log entries) — never supersede or overwrite it. To revise content that now lives only there, target the active page instead, or re-ingest the raw.
...
- The replacement should be re-derived from the raw, not taken on the user's word: supersede trusts the user-supplied replacement and does not fact-check it against a raw source — that grounding is `ingest`'s job (existing-source mode). Route there when faithfulness against the raw matters.
...
Pages: prior copy goes to `2-outputs/superseded/superseded-YYYY-MM-DD-HHMM-{filename}.md` (date-prefixed filename). Whole-page attachment snapshots: `2-outputs/superseded/attachments/{stem}/superseded-YYYY-MM-DD-HHMM/` (date as subfolder of {stem}, holding the full attachment set at that date). Single-file attachment replacements: `2-outputs/superseded/attachments/{stem}/superseded-YYYY-MM-DD-HHMM-{name}.{ext}` (date prefix on filename). Both attachment shapes group by {stem} so all prior versions of one source's attachments live together.

On a name clash in `2-outputs/superseded/` — a same-minute page supersession (a `superseded-...-{filename}.md` already present, e.g. a re-run after a partial failure) **or** two attachment replacements of the same `{name}` in one minute — apply the shared name-clash suffix rule in `.claude/skills/multi-skill/references/quarantine-path-convention.md` (so the second copy never overwrites the first prior view — the safety net itself), and point the replacement note at the suffixed name. The page path carries only a minute-resolution timestamp, so this clash guard is what protects it.
