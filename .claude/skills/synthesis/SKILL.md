...
6. **Draft approved synthesis page changes.**
   ...
   - New and updated synthesis pages normally land `status: draft`, set by the Step 8 checks under `verification.md` → Setting Status: a synthesis composes claims across several raws, and a run certifies only what it opened this pass, so anything it could not reach is marked `*[unverified]*` and the page stays `draft` rather than being stamped on partial certification. Editing a `verified` synthesis is claim-level for additions (CLAUDE.md → Page status): mark each newly-added non-obvious claim `*[unverified]*` and the page stays `verified` with that added claim as the pending delta; a change to an existing claim moves the hash and resets the page to `draft` (marking does not hold it `verified`).
...
8. **Verify against the raw sources, then read back before touching the log.** Synthesis pages are durable cross-source argument pages, so they get the same verification the ingest family runs, not a lighter self-check: run the claim check and page self-check in the shared `.claude/skills/multi-skill/references/verification.md` — the claim check's independence supplied by the tiered independent-refuter model (`verification.md` → Tiered Independent-Refuter Verification), at the tier each claim's impact sets — over the synthesis page and the raw sources its `Answer` draws on, reading the source-page-specific bullets as their synthesis analogues (the `Answer` / `Scope` / `Tensions` claims in place of a source page's TL;DR and Key Claims). Re-derive each load-bearing claim from the raw — not from the source page or the drafted answer — including at least one late-section detail as proof the raw was re-read this pass, and spot-check at least one `#page=N` deep-link per distinct raw. The claim check must come back clean in the same round; a fix re-runs it. Set the page's status by the spec's Setting Status rule: a synthesis composes across several raws, so unless this run opened every raw its claims cite, something on the page is uncertified — mark it `*[unverified]*` and leave the page `draft` for a later run to close, rather than stamping around it. Record both check results (the late-section detail, the `#page=N` spot-check, the claim tally, any fixes) in the Step 10 log entry. Beyond the shared packets, confirm the synthesis-specific points they do not cover:
...
- Verification: claim check clean, page self-check run (late-section detail re-read: {…}; #page=N spot-check: {physical page N + printed page seen, or n/a}; claims certified {N}, marked `*[unverified]*` {N}; fixes: {…or none}) — page left `status: draft` for audit.
