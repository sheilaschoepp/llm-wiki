...
- Pre-verification page checkpoint: after Step 4a closes and before Step 4b/7 content changes, run `.claude/skills/audit/scripts/capture_page_checkpoint.py 2-outputs/audit/baselines/audit-{run-id}-preverification.json --repo-root . --run-id {run-id} --warning-baseline-sha256 {baseline artifact SHA-256}` → status 0. Retain the exclusive canonical path and artifact SHA-256. Its complete page inventory must equal the Warning baseline and supplies every later rollback preimage.
...
- Reader execution plan: {input SHA-256; plan SHA-256; exact raw-manifest groups; planned evidence rows; planned calls; automatic waves; max 25 claims/4 pages per role call}
...
- Exact closure fields in the JSONL reconciliation row: canonical `preverification_checkpoint_path` and exact `preverification_checkpoint_sha256`, canonical `reader_execution_journal_path`, exact `reader_execution_journal_sha256`, and the arrays `warning_fingerprints`, `mention_occurrences`, `suppression_batches`, `suppression_reader_verdicts`, `neutral_page_transactions`, `reader_executions`, `reader_adjudications`, `destructive_correction_reviews`, `content_rollback_preimages`, and `infrastructure_failure_pages`. Non-mention dispositions are exactly `fixed|standing_ignore|verified_not_applicable|needs_update`. Mention semantic dispositions are exactly `genuine_wrap|accepted_suppression|graph_repair|graph_ignore`; frozen standing ignores are applied before enumeration and therefore have no occurrence disposition row. An identity change adds one old `superseded` row with `rekeyed_to` and one new `rekeyed` row with reciprocal `rekeyed_from` plus a semantic `final_disposition`; the relationship is bijective. Use `[]`; counts/prose do not substitute.
...
- Exhaustive negatives: {claim ID; verification_scope; coordinator-frozen quantified raw paths and members; each role's exact raw population, population, searched members, counterexamples, and search summary}. HOLD requires searched members to equal the frozen population.

## Reader batch artifacts
- Execution journal: `reader-artifacts/{run-id}/execution-journal.json`; {baseline SHA binding; journal file SHA-256; internal journal SHA-256; ordered entry count}. Initialize it and the independent `execution-anchors/` directory exclusively immediately after the Warning baseline. Before row import, each numbered execution gets one write-once `execution-anchors/execution-NNN.json` entry and one matching journal link. Anchor census, journal chain, report executions, and all direct artifact-root children agree exactly; a deleted journal cannot be reinitialized while anchor/run state remains.
- Numbered executions: Step 5 bullet execution(s), then Step 8 final-page/invalidated-bullet execution(s). For each: {execution number; canonical repo-relative `plan.json`, `sidecars/`, and `collected.json`; input/plan/collected SHA-256; bullet/page evidence-row counts; call count; raw-manifest group count; automatic wave count; all-record hash; ordered `terminal_row_ids`; ordered `superseded_rows`}. Each ordinary superseded row maps to a terminal same-role replacement; both unanimous REFUTE rows for a removed claim map to that claim's validated destructive-review row. Every on-disk `execution-NNN` directory is listed; invalidated spend is retained, never omitted.
- Receipts: {batch ID; role; agent/run identity; record count; sidecar path/SHA-256}. The coordinator receives these compact receipts, not full inline verdicts.
- Collection: `manage_reader_batches.py collect` → {status; collected artifact path/SHA-256; exact terminal calls and records}. Missing/extra/stale/pooled/truncated/non-independent sidecars are pending, never HOLD.
- Coordinator adjudication: one schema-exact `reader_adjudications` object per retained row with {record row ID; canonical record SHA-256; one consistent coordinator identity/run distinct from every retained reader identity/run; evidence rechecked true; quote re-extracted true for bullet HOLD/REFUTE and false for bullet CANNOT_CONFIRM/page rows; semantic decision; reasoning}. The coordinator reads every accepted full record, including superseded rows, in bounded slices before import; a clean receipt is not semantic evidence.
- Destructive review: one `destructive_correction_reviews` object for every removed or destructively rewritten claim whose two blind roles both REFUTE, containing both role row IDs and SHA-256 values, a third distinct agent/run blind to both roles, `decision: approve`, the exact correction, reasoning, canonical third-reader sidecar path, and exact sidecar file SHA-256. A mixed pair cannot authorize removal.
- Rollback proof: one `content_rollback_preimages` object per scoped page, sorted by path, with Boolean `preimage_existed`, exact post-Step-4a checkpoint bytes and SHA-256, checkpoint status/hash/marker count, and Boolean `rollback_required`. Existing paths use exact checkpoint bytes and `preimage_existed: true`; a new split-off path uses `false`, an empty byte string and its SHA-256, null prior status/hash, and zero markers, authenticated by absence from the checkpoint whose inventory equals Step 1. Terminal reports require every rollback flag false. An infrastructure-aborted report lists exactly the true paths in `infrastructure_failure_pages`, restores existing checkpoint bytes, removes new paths, and passes `--verify-incomplete-rollback`.
- Terminal binding: the reconciliation's `reader_executions` objects are schema-exact and the completion validator replays every retained sidecar. Terminal rows alone must equal all final current `bullet_verdict` and `page_reader` rows exactly; superseded rows remain authenticated cost.
...
{"schema_version":1,"row_type":"page_reader","row_id":"page-locator-row","run_id":"...","relationship_epoch":"READY(n)","page_path":"1-wiki/concepts/example.md","page_generation":"...","raw_manifest":[{"raw_path":"0-raw/papers/example.pdf","sha256":"64 lowercase hex"}],"role":"locator_page","agent_id":"page-locator-agent","blind_to":["entailment_argument_page"],"verdict":"hold","defects":[],"evidence":"..."}
{"schema_version":1,"row_type":"page_reader","row_id":"page-entailment-row","run_id":"...","relationship_epoch":"READY(n)","page_path":"1-wiki/concepts/example.md","page_generation":"...","raw_manifest":[{"raw_path":"0-raw/papers/example.pdf","sha256":"64 lowercase hex"}],"role":"entailment_argument_page","agent_id":"page-entailment-agent","blind_to":["locator_page"],"verdict":"hold","defects":[],"evidence":"..."}
...
{"schema_version":1,"row_type":"reconciliation","row_id":"reconciliation-row","run_id":"run-id","result":"complete","pending":0,"warning_baseline_path":"2-outputs/audit/baselines/audit-run-id.json","warning_baseline_sha256":"64 lowercase hex","warning_baseline_id":"non-placeholder baseline ID","evidence_context_sha256":"64 lowercase hex","preverification_checkpoint_path":"2-outputs/audit/baselines/audit-run-id-preverification.json","preverification_checkpoint_sha256":"64 lowercase hex","reader_execution_journal_path":"2-outputs/audit/reader-artifacts/run-id/execution-journal.json","reader_execution_journal_sha256":"64 lowercase hex","planned_pages":1,"terminal_pages":1,"pending_pages":0,"planned_sources":1,"terminal_sources":1,"pending_sources":0,"planned_claims":1,"terminal_claims":1,"pending_claims":0,"planned_bullet_roles":2,"terminal_bullet_roles":2,"pending_bullet_roles":0,"planned_page_readers":2,"terminal_page_readers":2,"pending_page_readers":0,"planned_scanners":1,"terminal_scanners":1,"pending_scanners":0,"planned_status_writes":1,"terminal_status_writes":1,"pending_status_writes":0,"initial_warning_findings":0,"initial_nonmention_warning_fingerprints":0,"initial_mention_groups":0,"expanded_mention_occurrences":0,"zero_match_scanner_defects":0,"introduced_warning_findings":0,"introduced_nonmention_warning_fingerprints":0,"introduced_mention_groups":0,"introduced_mention_occurrences":0,"terminal_nonmention_warning_fingerprints":0,"pending_nonmention_warning_fingerprints":0,"terminal_mention_occurrences":0,"pending_mention_occurrences":0,"warning_fingerprints":[],"mention_occurrences":[],"suppression_batches":[],"suppression_reader_verdicts":[],"neutral_page_transactions":[],"reader_executions":[{"execution_number":1,"plan_path":"2-outputs/audit/reader-artifacts/run-id/execution-001/plan.json","plan_sha256":"64 lowercase hex","input_sha256":"64 lowercase hex","planned_groups":1,"planned_calls":2,"planned_waves":1,"planned_records":2,"artifact_dir":"2-outputs/audit/reader-artifacts/run-id/execution-001/sidecars","collected_path":"2-outputs/audit/reader-artifacts/run-id/execution-001/collected.json","collected_sha256":"64 lowercase hex","terminal_calls":2,"terminal_records":2,"terminal_row_ids":["locator-row","entailment-row"],"superseded_rows":[],"records_sha256":"64 lowercase hex"},{"execution_number":2,"plan_path":"2-outputs/audit/reader-artifacts/run-id/execution-002/plan.json","plan_sha256":"64 lowercase hex","input_sha256":"64 lowercase hex","planned_groups":1,"planned_calls":2,"planned_waves":1,"planned_records":2,"artifact_dir":"2-outputs/audit/reader-artifacts/run-id/execution-002/sidecars","collected_path":"2-outputs/audit/reader-artifacts/run-id/execution-002/collected.json","collected_sha256":"64 lowercase hex","terminal_calls":2,"terminal_records":2,"terminal_row_ids":["page-locator-row","page-entailment-row"],"superseded_rows":[],"records_sha256":"64 lowercase hex"}],"reader_adjudications":[{"schema_version":1,"row_id":"adjudication-locator","run_id":"run-id","record_row_id":"locator-row","record_sha256":"64 lowercase hex","coordinator_id":"audit-coordinator","coordinator_run_id":"coordinator-run-id","evidence_rechecked":true,"quote_reextracted":true,"semantic_decision":"hold","reasoning":"The exact located quote supports the claim."},{"schema_version":1,"row_id":"adjudication-entailment","run_id":"run-id","record_row_id":"entailment-row","record_sha256":"64 lowercase hex","coordinator_id":"audit-coordinator","coordinator_run_id":"coordinator-run-id","evidence_rechecked":true,"quote_reextracted":true,"semantic_decision":"hold","reasoning":"The exact located quote entails the scoped claim."},{"schema_version":1,"row_id":"adjudication-page-locator","run_id":"run-id","record_row_id":"page-locator-row","record_sha256":"64 lowercase hex","coordinator_id":"audit-coordinator","coordinator_run_id":"coordinator-run-id","evidence_rechecked":true,"quote_reextracted":false,"semantic_decision":"hold","reasoning":"The complete page and raw manifest contain no locator defect."},{"schema_version":1,"row_id":"adjudication-page-entailment","run_id":"run-id","record_row_id":"page-entailment-row","record_sha256":"64 lowercase hex","coordinator_id":"audit-coordinator","coordinator_run_id":"coordinator-run-id","evidence_rechecked":true,"quote_reextracted":false,"semantic_decision":"hold","reasoning":"The complete page and raw manifest contain no argument defect."}],"destructive_correction_reviews":[],"content_rollback_preimages":[{"page_path":"1-wiki/concepts/example.md","preimage_existed":true,"preimage_sha256":"e55c700d94764f70a58d1fb19d7239875cee2d82036f2bdafe7f8e96a360e614","preimage_bytes_base64":"LS0tCnR5cGU6IGNvbmNlcHQKc3RhdHVzOiBkcmFmdAotLS0KQm9keS4K","before_status":"draft","before_verified_hash":null,"process_marker_count":0,"rollback_required":false}],"infrastructure_failure_pages":[]}
```
