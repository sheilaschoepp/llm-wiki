...
- `dir_tree_drift` — parses the ASCII tree in `CLAUDE.md` and compares it to the repo. The tree is the first `text`-fenced block holding `├──` / `└──` branches, identified structurally rather than by its heading or its root line, so the check works in a repo whose root folder is named something other than the one the tree shows. Conventionally that block sits under `## Directory structure`, but the heading is not what locates it. Flags tree entries absent on disk and on-disk paths the tree should list but omits. Root-level proposals.
