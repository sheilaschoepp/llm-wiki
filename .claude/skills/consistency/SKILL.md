---
name: consistency
description: Run project-level consistency checks after schema or skill changes. Checks CLAUDE.md, README.md, skill files, wiki page section templates, index-vs-files gaps, callout CSS, and scripts for drift from its schema. Use after a schema change, skill rewrite, feature removal, or major refactor. Also use when the user asks whether a refactor broke anything, whether the project is consistent with its schema, whether a skill or its description still matches the schema after editing a SKILL.md or CLAUDE.md, or to propagate a renamed or moved section, callout, or field across the project. Different from lint and audit, which check individual wiki pages (lint structurally, audit semantically); from cleanup, which checks memory-entry content and prunes unneeded outputs (consistency only counts memory entries against a cap); and from skill-linter, which reviews one skill's authoring quality. Auto-fixes mechanical drift; never edits CLAUDE.md, skills, or scripts — those are proposals.
---
...
- `1-wiki/hot.md`, `1-wiki/index.md`, `1-wiki/log.md` — only `index.md` gets a structural check (`index_vs_files_drift`); `hot.md` and `log.md` are read for path presence in the directory tree, and their internal structure is `lint`'s
- `MEMORY.md` and `2-outputs/` — read by `memory_file_graduation_prompt`, `filename_references_resolve`, and `file_naming_consistency`
- `a-archive/` — read-only here: `filename_references_resolve` and `ai_writing_tells` scan it, but findings are surfaced, never auto-fixed
...
5. **Fix mechanical drift.**
   - **Freeze the schema-derived fixes while the schema is in dispute.** If `section_lists_match_schema` fired this run, apply no `EXPECTED_SECTIONS`-derived fix — `body_section_order`, `callout_css_coverage`, placeholder normalization — and surface each as deferred instead. That finding means the script's hardcoded roster and `CLAUDE.md` disagree, so the derived fixes are pointing at a template that may be the retired one; the finding is self-contradictory on its face (its `fix_hint` says match `CLAUDE.md` while its `message` prints the stale expectation), which is exactly why neither direction can be trusted until the roster question is settled as a root-level proposal.
   - Update stale section names in wiki pages. A section-name or callout-order fix is a pure rename/move — never alter callout body content or block IDs while doing it. Before applying any body-mutating wiki-page fix — a `body_section_order` reorder/rename or a `placeholder_consistency` normalization, both of which rewrite the hashed body — first read the target page's `status:` frontmatter (the script's finding does not carry it; its fix_hint says only "Reorder body callouts…", so the status read is the agent's job). If the page is `status: verified`, do not edit it: applying the fix would trip the body-hash and demote the page without a record (consistency is not among the re-stamping skills). Surface such a page as a finding for `lint`/`audit` to handle under the verification model rather than editing a verified body here.
   - Add missing callout CSS for new slugs.
   - Update README if the user-facing summary no longer matches `CLAUDE.md`.
   - Skill descriptions that mention old citation behaviour, and scripts that no longer match the required sections, are skill/script drift — a Step 6 proposal, not an auto-fix (the autonomy boundary is stated in Limits).

6. **Prepare root-level findings as concrete proposals.** The never-auto-fix rule itself is stated once, canonically, in Limits. The boundary is the decision type, not the file: a README or wiki-page edit that would settle a schema question is a proposal too. For each root-level finding, and for any judgement call that could be resolved by changing the schema or a skill, prepare a concrete proposed change: the exact edit, not just a description of the drift. Do not ask mid-run; these proposals are presented together at the end (Step 10).

   - Autonomy boundary (the base rule — never auto-edit `CLAUDE.md`, a skill file, or a script — is stated once, canonically, in Limits). What this step adds is the decision-type nuance: the boundary is decision type, not only file identity — a change to an auto-fixable file (README, a control page) that would settle a schema or skill question — rewording a user-facing behavioural claim, choosing between two readings of the schema — is a proposal, not an auto-fix; when a finding's target is genuinely ambiguous (a control file that is part-regenerated, part-curated), default to a proposal.

7. **Iterate independent re-runs until two consecutive passes are clean.** This is a loop, not a single closing re-run:

   1. Re-run the full deterministic script as one independent pass: a fresh complete run over the whole project, not an incremental re-check of only the lines you just edited. A mechanical fix can break a check it did not touch, so the pass must be whole. Capture the exit code — the gate reads it, not stdout (in a crash, stdout is a normal-looking findings array and only the exit code, or a `file: (internal)` finding, reveals `blocked`; see Step 8):

   ```bash
   python3 .claude/skills/consistency/scripts/check_consistency.py . ; echo "exit=$?"
   ```
   ...
   3. A pass is clean when an independent full pass leaves the auto-fixable partition empty (the script still exits 1 while non-blocking findings remain — exit 1 is not the not-clean signal). Five kinds of finding do not block convergence, because the skill cannot resolve them itself:
      - surfaced judgement calls;
      - surfaced root-level proposals — classify these by the finding's target `file`, not a memorized check-id list: a finding whose `file` is `CLAUDE.md` or any path under `.claude/skills/` (a skill file or a script) is root-level, never auto-fixed;
      - a finding whose fix a **named** rule forbids this run to apply: an edit to a file the skill may not edit, or an action CLAUDE.md → Safety rules gates on the user (a rename, a folder move, a deletion). The run must cite the specific Limits line or Safety clause, and list the finding's `check_id`, `file`, and the cited rule under `## Proposals` in the report. **Absent a citable rule the finding is ordinary auto-fixable drift and blocks** — this class is not a place to park work the run would rather not do. Clean-compatible, surfaced once, never auto-applied. A *recurring* member of this class means a check is tagged wrongly in `references/checks.md`, which is itself raised as a root-level proposal;
      - findings recorded as sanctioned in `consistency-memory.md` (e.g. a `domain_literature_leakage` hit on content the memory marks accepted) — surfaced as sanctioned, never as drift and never as a proposal to remove them;
      - advisory findings the skill only counts and is structurally forbidden from acting on — a `memory_file_graduation_prompt` over-cap notice on `MEMORY.md` (memory graduation is `cleanup`'s job, not consistency's) or an `identity_term_leakage` inactive-source notice (the advisory the check emits when `a-archive/about-me/about-me.md`'s `## Identity` section is missing or unparseable, so the identity scan ran against no terms) — surfaced for the user but never blocking, since consistency cannot clear them itself;
      - a wiki-page callout/section-order fix consistency declines because the target page is `status: verified` (Step 5): it is deferred to `lint`/`audit`, surfaced once, and it is clean-compatible — it does not spin the loop and does not by itself force `result: findings`. A verified page's callouts lagging an already-agreed schema is page-level structural drift (`lint`'s half of audit's precondition), not a `section_lists_match_schema` schema-integrity dispute (the schema disagreeing with itself); forcing `findings` here would gate off the very `audit` that Step 5 defers it to.

      An `(internal)` finding is NOT in this set: it is a crash or manifest-wiring failure, so the battery did not run to completion — it forces `result: blocked` and aborts the loop (see Step 8), never a settled terminal state. Surface each non-blocking finding once; an unchanged set of them across passes is the settled terminal state, not a reason to keep looping. The pass is `clean except for surfaced proposals` — report the pass as clean (its auto-fixable partition is empty) and carry the proposals to Step 10. One nuance carries to Step 8: a *schema-integrity* proposal (`section_lists_match_schema` or a judgment-drift contradiction) still does not block the loop from converging, but it does set the Step 8 `result:` field to `findings` rather than `clean`, because audit must not run against a schema in dispute.
   ...
   Bound the loop: stop after 6 iterations, or earlier when an iteration makes no progress on the *auto-fixable* partition (a fix leaves the same auto-fixable finding standing, or fixes oscillate). An unchanged set of the non-blocking classes above (proposals, sanctioned hits, advisories, a deferred verified-page finding) is the settled terminal state, not no-progress — do not bound out on it. When genuinely bounded out without reaching two consecutive clean passes, stop and report the unresolved findings as `Result: blocked` rather than continuing to spin.
...
8. **Save report** to `2-outputs/consistency/consistency-YYYY-MM-DD-HHMM.md`. Obtain the timestamp at write time with `TZ='UTC' date '+%Y-%m-%d-%H%M'` — the session context gives the date but not the current minute. If a consistency report already exists for the same minute, do not overwrite it — append a suffix (`-rerun`, `-after-fixes`, or an ordinal `-2`/`-3`), mirroring lint, so a same-minute re-run never silently clobbers the prior report and a standalone `audit` reads an unambiguous newest. A bounded-out run (Step 7) still writes this report and the Step 9 log entry before stopping, so the newest report on disk reflects the `blocked` state, not a stale prior clean one.
   ...
   The report must open with this frontmatter — `audit` gates its project-level precondition on the `result:` field (the same way it gates Step 1 on lint's), stopping on `findings` or `blocked`. This report-level `result:` is one derived verdict for the whole battery — distinct from the per-packet `Result:` blocks of Step 3, and computed from them (Step 4 merge) plus the carve-outs below, not copied from any single packet:
   ...
   The report carries no per-severity counts. Findings do not carry a severity — `finding()` emits `check_id`, `file`, `line`, `message`, `fix_hint` and nothing else — so any `errors:`/`warnings:`/`suggestions:` tally would be invented per run rather than derived, and nothing consumes it (`audit` gates on `result:` alone). The one place severity genuinely exists is `AI_TELL_PATTERNS`, which carries it per pattern in this skill's `error / warning / suggestion` vocabulary; that mapping belongs in `references/checks.md` under `ai_writing_tells`, not in a frontmatter total. Set `result: clean` when the loop reached two consecutive clean passes (Step 7) with the auto-fixable partition empty, the final independent pass exited 0 with no `(internal)` finding on stdout (a crashed check or broken wiring exits 2 and forces `blocked` — a crash must never be read as a settled clean pass), the judgment-drift packet was actually performed (`judgment_drift: performed`), and no *schema-integrity* proposal is outstanding — an ordinary surfaced root-level proposal (a wording fix, a new skill) does not block `clean`, it is counted in `proposals` (a clean report can carry `proposals:` > 0). Two carve-outs force `findings` instead of `clean`, because each means `audit` would run against ground its precondition exists to protect: (a) an unresolved *schema-integrity* proposal — a `section_lists_match_schema` finding (the `CLAUDE.md` callout templates vs the script's `EXPECTED_SECTIONS`) or a judgment-drift contradiction — says the schema is itself internally inconsistent, so auditing pages against it is exactly what the gate must block; (b) a `judgment_drift: skipped` attestation — that packet is a model read with no deterministic script backstop, so a skipped read must not pass as `clean` on the strength of an exit-0 battery that never covered it. Findings recorded as sanctioned in `consistency-memory.md` are likewise subtracted before computing `result:` and the `errors`/`warnings`/`suggestions` counts: a run whose only non-clean findings are sanctioned (or root-level proposals) is `clean`, with the sanctioned instances listed in the report body under a `Sanctioned (accepted, not drift)` note citing the memory entry — not counted as drift, never proposed for removal. A sanction entry in `consistency-memory.md` is an H2 section — the `check_id`, the exact accepted `file` or string, and a one-line rationale (e.g. `## Sanctioned: domain_literature_leakage — <bibkey> in <path>`, followed by an `Accepted: …` reason line) — which the run matches findings against before subtracting them. Subtract a finding only when it matches a specific `consistency-memory.md` entry cited in the report; an over-broad or unmatched sanction claim is surfaced as drift, not silently subtracted (the match is an agent step with no script backstop, so it must be evidenced by the cited entry). The script exits 1 on any finding (including standing proposals and sanctioned hits), so exit 1 is not the not-clean signal. Set `findings` when unsanctioned auto-fixable drift remains unresolved, and `blocked` when the battery did not run to completion — script exit code 2 (a check crashed: an `(internal)` finding on stdout while the rest of the battery still ran, so its coverage is incomplete), or the loop bounded out (Step 7) without two consecutive clean passes.

Capture the script's exit code with `$?` after the final independent pass; do not infer the gate from stdout alone. **The exit code is necessary but never sufficient for `clean`** — the conditions above are the single authority, and two of them (`judgment_drift: performed`, no outstanding schema-integrity proposal) are invisible to any exit code, because the judgment-drift packet has no script. So an exit-0 battery whose model read was skipped is `findings`, not `clean`. Exit 2 ⇒ `blocked`, always: a crashed check, broken manifest wiring, OR a malformed invocation (bad path / unknown check / a comma- or whitespace-only `--checks` that resolves to no checks / bad `--packet`). The two exit-2 cases are told apart by stdout: a crashed check or wiring error prints a populated JSON array carrying an `(internal)` finding (the rest of the battery still ran, so coverage is incomplete), while a malformed invocation prints nothing to stdout and a message to stderr. Neither an `(internal)` finding nor an empty or absent JSON array may ever be read as `clean` — that is the trap, since the gate audit depends on would silently pass over a battery that never ran. Exit 1 ⇒ `findings` when unsanctioned auto-fixable drift remains; when the only residue is the non-blocking classes in Step 7, exit 1 is compatible with `clean` but does not establish it — the conditions above still decide (proposals are themselves findings, so a clean-but-with-proposals run exits 1, not 0). Exit 0 likewise permits `clean` without establishing it. Any loop iteration ending bounded-out or with unresolved auto-fixable drift is `blocked`/`findings`, never `clean`, even if a single pass momentarily showed an empty partition. The enum differs from lint's `clean | blocking`; `audit` reads each skill's own vocabulary. This contract is recorded in `CLAUDE.md` → Audit preconditions and `audit`'s Step 2.
