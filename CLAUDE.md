...
**Earning `verified` requires the tiered independent-refuter gate.** A wiki claim reaches `verified` only after passing an independent-refuter verification tiered by the claim's impact: obvious / own-voice / mechanical content needs no refuter, a low-stakes non-obvious claim needs one, and every other non-obvious claim needs two **role-specialized** refuters that must both hold — the broad default, so in practice most claims. The two roles are complementary, not duplicate: a **locator** refuter asks whether the cited content sits at the cited page under the anchor named, and an **entailment** refuter asks whether that evidence supports the claim's exact wording (scope, strength, attributed mechanism, the subject a position is predicated of). Count is not the lever — refuter errors correlate, so two different lenses cover more than three identical reads, and each extra refuter is another chance at a fabricated objection that overwrites correct content. A **third** refuter is spent only on a named escalation trigger: an evidence-backed split, a destructive or unbounded correction (bullet removal), or a load-bearing aggregate whose cold recomputation contradicts the cited cell; an unreadable raw region is not a trigger, since a third refuter cannot read what two could not. The refuters read and reason only — spawned with read-only tools from the orchestrating agent, which applies all status itself, never nested inside another subagent — and each attempts to refute the claim against the raw, returning a structured verdict (`refuted` / `cannot confirm` / `no valid refutation found`) with a verbatim quote and physical page. The orchestrator adjudicates on evidence, not verdict count: it re-greps each returned quote against the raw and discards any finding whose quote does not verify. The claim is certified only on agreement after adjudication, and a surviving disagreement routes the page to `needs-update`. For a summary or generalization claim the entailment refuter recomputes the whole set the claim ranges over, not the one cell it cites. This gate is where `verified` is earned — `audit`'s job, and the only place the stamp is set — and it is defined once, with the full mechanics, in `.claude/skills/multi-skill/references/verification.md` (Tiered Independent-Refuter Verification); every content-authoring skill uses that one spec. Authoring skills (`ingest`, `synthesis`, `query`, `supersede`) finish at `draft` and run the authoring tier defined in `.claude/skills/multi-skill/references/verification.md` — the packets plus one refuter per owed unit, never the pair; the certifying pair runs once, when `audit` earns `verified`. The `verified_hash:` mechanism below is unchanged — the refuter gate certifies the claim against the raw, the hash certifies the checked body has not changed since.
...
**`audit`'s two claim-scoped dispositions.** `audit` itself is not bound to the whole-page route: when it confirms and fixes a defect during a run, on a page whose pre-edit body still matches its `verified_hash:`, it may take one of two claim-scoped dispositions instead of demoting. (1) Delta-certify: fix the claim, certify that claim through its impact tier's independent refuters this run, re-stamp — the hash proves every untouched line byte-identical to the last certified body, so the unchanged claims keep their prior certification. (2) Fix-and-mask: fix the claim, mark that one line `*[unverified]*`, re-stamp the masked hash — the rewritten line is explicitly uncertified and rides as the pending delta a later `partial` certifies through its tier; audit certifies nothing it wrote this run. Guards on both: the pre-edit hash must match the stamp (else the page drifted out of band and takes the whole-page route); a fix that adds or relabels a raw-locator structural anchor rides only the marked path; every summary, aggregate, or generalization claim on the page re-runs its tier whenever any claim on the page was added, removed, or changed (the delta may have moved the set it ranges over); a same-page bullet that leans on the fixed wording is re-read and treated as changed if it no longer holds (the delta-collateral sweep); and the distortion-disposition rule is untouched — an unbounded or multi-page correction still routes to `needs-update`. Marker-clearing always runs the claim's refuter tier — a cleared marker is a certification, never a same-agent read.
