...
- Locating the figure box (PyMuPDF coordinate method):
  ...
  - A figure's graphics come from two independent calls, and either may be empty: `page.get_drawings()` returns vector rects, and `page.get_image_rects(xref)` over `page.get_images(full=True)` returns embedded-raster placements. A photographic or screenshot figure has raster panels and no vector drawings at all, so a box built from `get_drawings()` alone silently returns the caption and whatever vector rules sit near it. Union both sources.
  - Exclude stray drawing rects before unioning — **vector rects only**: the page frame (height ≥ 700 pt), hairlines/rules (height < 3 pt), and any rect that extends beyond the page bounds or is larger than ~0.6 of the page — these are clip-path/background artifacts that otherwise balloon the figure bbox. Do not apply these tests to raster placements: a full-page raster *figure* trips both the height and the area test, which drops exactly the figure that has no vector rects to fall back on.
  - A raster that covers the whole clip is the page substrate of a **scan**, not a figure panel. It intersects every possible sub-page clip, so it is excluded from the candidate set; when it is the only graphics on the page, coordinate isolation has nothing to work with — say so and fall back to the render-then-crop loop rather than certifying a crop the checks never examined.
...
- Double-check every crop before finalizing — by coordinates, not just a downscaled glance. The check must be **independent of the box-building step**: re-collect the page's candidate rects fresh and test the clip against them. Confirming that the clip contains the union it was *built from* is a tautology that passes on every crop, including the caption-only crop a missed raster panel produces — the one failure the check exists to catch. Three tests: no candidates at all means the box collapsed onto the caption; a candidate crossing the clip boundary means a panel is cut in half; a candidate just outside it (within ~40 pt) may be a panel left behind, and is resolved before rendering — widen the clip, or record the position reading against the caption band that shows it is unrelated. Raise a narrow error rather than a bare `assert`, which `python -O` strips. Then confirm item by item: the whole figure (all panels, all axes, the full legend); the full caption (every line plus the `Figure N` / `Table N` label); and nothing extraneous (no page header or header rule, no adjacent-column text, no body paragraph above or below, no clipped title or legend). A plausible-looking crop that clips half a figure or grabs a header is a silent defect the later verification passes do not catch.
...
**Earning `verified` requires two evidence layers.** Every required non-obvious bullet has one blind locator HOLD and one blind entailment HOLD under the exact identity/reuse contract in `.claude/skills/multi-skill/references/verification-ledger.md`; obvious/definitional, own-voice, empty-placeholder, and verification-neutral bullets remain explicit exemptions. Locator and entailment are complementary, not duplicate. `ingest` creates durable bullet evidence but finishes at `draft`; the other authoring paths keep their existing non-reusable authoring smoke test until separately migrated. `audit partial` may reuse a pair only from a committed, clean, terminal ingest/audit report when the full bullet, ordered locators, raw SHA-256 values, context/dependencies, and role/schema versions still match; it backfills both roles for anything else. `audit full` is cold and reruns both roles for every required claim. Every audited page then receives two fresh blind whole-page gates on its final body and raws: locator/source assignment and entailment/argument consistency. Neither layer substitutes for the other, and any surviving non-HOLD blocks promotion. The coordinator rechecks literal quotes at the attributed raw page and adjudicates evidence, never verdict count. A third reader remains limited to evidence-backed splits, destructive/unbounded corrections, or aggregates whose recomputation changes the answer. Only `audit` stamps `verified`; the refuter evidence certifies claims against raws and `verified_hash:` certifies that the checked body has not changed.
...
**Changing an existing verified claim demotes the whole page to `draft` — marking it does not hold the page `verified`.** The asymmetry is mechanical, not a judgement call, and `body_hash.py` makes it exact: an *added* claim marked `*[unverified]*` was never part of `verified_hash` (a masked new line is invisible to the hash), so the page stays `verified`; but a *changed* claim was already in the hash (it was unmarked and verified), so altering its text — or marking it, which removes its line from the hashed body — moves the hash, and `lint`'s hash check resets the page to `draft` for a later `audit` to re-verify. So an addition rides as a marked pending claim; a change made *outside* `audit`'s own in-pass fact-check re-verifies the whole page — the demotion is what routes the page into a later audit's whole-page scope. Beyond `audit`'s two claim-scoped dispositions (next paragraph), the one further exception is a pure move: relocating a claim whose text stays byte-identical and whose meaning its new position does not change is a verification-neutral edit the moving skill re-stamps (see below), so a simple move keeps the page `verified`.

**`audit`'s two claim-scoped dispositions.** On a hash-stable verified page, audit may delta-certify a bounded fixed claim through both bullet roles and both fresh page gates, then re-stamp; or fix-and-mask it with `*[unverified]*` and re-stamp the checked remainder for a later partial. The pre-edit hash must match. New/relabelled locator anchors stay marked until the anchor is at Git HEAD. Aggregate/context-dependent and collateral claims invalidate according to their declared dependencies or conservative full-page digest. Marker clearance requires an exact-valid bullet pair, both fresh final-page HOLDs, a current epoch, HEAD-mature anchor, structural pass, and an atomic marker/status/hash write that changes no other semantic byte.

A scope-widening change to `frames:` — one where the new union admits source material the old union did not (a strict superset) — still resets the page to `draft`: it grows the unmarked body, so the whole page needs re-checking. A change that admits nothing new (a reword, or a lens the body already covers) and a pure field migration that preserves the same scope do not. Mechanical metadata updates — `updated:`, `source_count`, index and log bookkeeping — do not change status. `audit partial` (default) scopes every non-verified page, every `*[unverified]*` claim on a verified page, and pages current defects/raw invalidation pull in; it may reuse only exact-valid bullet pairs and always pays two fresh page gates per scoped page. `audit full` scopes every claim/page and reruns both bullet roles cold (`reused = 0`) before those page gates. Routine `/checkup` uses partial.

`audit` enforces verification with a content stamp scoped to the checked content: when it sets a page `verified` it writes `verified_hash:` to the frontmatter — the SHA-256 of the page body **excluding every `*[unverified]*`-marked claim**. The stamp is written only after bullet evidence, both fresh page gates, and terminal reconciliation pass. The exclusion is what makes the claim-level model work:
...
The hash covers the body (minus marked claims) only, so mechanical frontmatter updates do not trip it. `lint` and `audit` both compute the hash with `.claude/skills/multi-skill/scripts/body_hash.py`, which masks `*[unverified]*` claims before hashing.
...
So the rule for editing a `verified` page is: **mark each newly-added non-obvious claim `*[unverified]*`** and **let a change to an existing claim demote the page**. If verified content changes outside audit and is neither a marked addition nor an allowlisted verification-neutral edit, lint demotes it. Two re-stamps are legitimate: (1) audit's terminal write after exact-valid bullet evidence, both fresh final-page HOLDs, marker/HEAD eligibility, structural success, and reconciliation; and (2) an allowlisted verification-neutral edit. Never re-stamp by hand after another unmarked body edit. Source/support attachment is never claim-neutral: it demotes and strips the old hash regardless of reusable bullet rows.

## Bullet Markers

Most bullets carry no marker. Use markers sparingly, and always in canonical position — immediately after the bullet's `> - ` prefix, before the claim text:

```markdown
> - *[unverified]* the claim text
```

- `*\[unverified\]*` for a non-obvious claim awaiting a raw fact-check. Added when a *new* claim is inserted into a `verified` page without checking it against the raw in the same pass, so the page stays `verified` while only that added claim is the pending delta (see Page Status). Marking a *changed* existing claim does not keep the page `verified` — a change moves the hash and demotes the page to `draft` — so the marker is for additions, not changes. The one exception to "additions, not changes" is `audit`'s own fix-and-mask disposition (Page Status): when `audit` fixes a confirmed defect confined to one line on a page whose body still matches its stamp, it may mark that line and re-stamp the masked hash — the fixed claim is then a pending change a later `partial` certifies through an exact-valid locator/entailment bullet pair plus both fresh page gates; no other skill, and no hand edit, may do this. `audit` removes it once that terminal gate holds. Distinct from `*\[tentative\]*`: `*\[unverified\]*` is a process state (not yet checked, removed on check); `*\[tentative\]*` is an epistemic judgement (support is weak) that persists even after verification. A claim can carry both. Not used on `draft` pages, where the page status already means everything is unverified. Position is load-bearing, not cosmetic: both `body_hash.py`'s masking and `lint`'s pending-claim count key on the canonical position only, so a marker written elsewhere leaves the claim inside the page's verified hash *and* invisible to the burndown — the claim reads as pending to a human and as checked to every tool.
...
- Every operation that authors or changes durable `1-wiki/` content runs the shared two-packet verification in `.claude/skills/multi-skill/references/verification.md` before finalizing. `ingest` additionally writes paired per-bullet evidence under `verification-ledger.md`; query promotion, synthesis, and supersede keep their existing one-reader authoring smoke test and produce no reusable ledger credit until separately migrated. Authoring paths keep new pages `draft`; a verified page may carry marked additions while changed claims demote. Authoring evidence never stamps verified. Audit is the independent second layer: partial may admit only exact-valid committed ingest/audit bullet pairs, backfills both roles otherwise, and runs two fresh blind whole-page gates on every scoped page; full reruns every bullet pair cold before the same gates. Read-only/output-only skills author no durable wiki content and run no packets; forget removes rather than authors.
...
- Use the `AskUserQuestion` tool for every user-facing decision during write operations (`ingest`, `synthesis`, `supersede`). Each question marks the option you recommend `(Recommended)` and orders it first — reflecting the skill's own read of the source — unless you genuinely have no lean, which you state.
- **Batch as far as the decisions allow.** A run that fires twenty separate calls costs twenty round trips of the whole conversation, and a slate the user could have answered in one pass is the commonest source of that. The tool takes up to four questions per call, each with up to four options, and `multiSelect`. That gives two batching forms, and the candidate's choice shape picks which:
    - **Binary include / drop → one `multiSelect` question, one option per candidate.** Selecting is including; up to four candidates per question and four questions per call, so sixteen fit in one call. Concept/entity page candidates, image include/drop, retrieval tags, and sibling back-references are the standard slates.
    - **A per-candidate choice with three or more outcomes (include / soften / drop, keep / replace / remove) → one question per candidate, up to four questions per call.** `multiSelect` cannot express it, but the call still carries four decisions instead of one.
  In both forms keep each candidate's rationale in its option `description` and the `(Recommended)` marking per option — batching must not cost the user the skill's read on each item. Batch only where the candidates are independent and no answer changes what the next question asks.
- **Some decisions cannot batch, and forcing them into a slate is worse than the round trip.** Ask on its own: a decision whose answer changes what is asked next; one needing free-form input (a frame text, a book page range, a page name); ambiguous source identity or stem resolution; an unexpected scope change; a contradiction (each needs both sides laid out); a relationship-sweep edge (each asserts a distinct thing about the graph); a deletion that is not git-recoverable (see Safety rules — those stay strictly per file); and any decision whose recommendation needs more explanation than an option description holds. When in doubt, ask separately: a batched decision the user misreads is more expensive than the call it saved.
...
- Use the `AskUserQuestion` tool whenever you'd otherwise offer 2–4 choices or list numbered options — in any context, including inside skill runs. Always indicate your recommendation: order your recommended option first and mark it `(Recommended)` in its label. The sole exception is a genuine no-lean choice (a pure user-preference call with no better answer) — there, state that you have no recommendation rather than faking a pick. This applies everywhere, including the per-decision questions inside write operations (see **Workflow Rules**).
- **Every open question goes to the user as a detailed `AskUserQuestion`, never as prose.** An open question is anything left genuinely undecided that is the user's call — a pending approval, an unresolved trade-off, a "what next" at the end of a run, a blocker, or a choice the work surfaced and did not settle. Do not raise one as a paragraph, a bulleted list, or a closing "let me know which you'd prefer": if it is open and it is theirs to decide, it is a question with options. This overrides the older allowance for handling a single open clarification in plain prose.
    - **One item per question.** A question covers exactly one unresolved item. Never bundle several items into one question and offer "all / some / none" as the options — that forces a single answer onto decisions the user may want to take differently, and it hides each item's own trade-off behind a count. Options are the ways to resolve *that* item; if two items need separate answers, they are two questions.
    - **Simple terms.** Write the question and every option in plain language: short sentences, no jargon, no internal identifiers (step numbers, check IDs, file-and-line references) unless the decision genuinely turns on one. Say what goes wrong and what the fix does, in the words a reader who has not seen the code would use. A question the user has to decode before answering has failed, however precise it is.
    - **Detailed** is the operative word. Each option's `description` states what will actually happen if chosen and the trade-off that distinguishes it from its neighbours — enough that the user can decide without scrolling back or asking a follow-up. An option gloss that only restates its own label is not detailed. Detailed and simple are not in tension: detail belongs in *what the choice means*, never in vocabulary.
    - Batch per **Workflow Rules → Batch as far as the decisions allow**: up to four questions per call, `multiSelect` where the choices are not mutually exclusive. Batching groups whole questions into one call; it never merges two items into one question (see **One item per question** above).
    - This does not widen *what* gets asked. Editorial micro-decisions inside a run are still made autonomously and stated for after-the-fact correction (see **Workflow Rules**), and a question you can answer yourself from the code, the files, or a sensible default is not an open question — answer it and say what you assumed. The rule governs the form of a question already worth asking, not a licence to offload judgement.
...
- `.claude/skills/` is soft read-only. The agent-writable exceptions are per-skill and multi-skill memory journals plus the curated runtime data files `hyphenation-lists.md`, `unlinked-mention-ignore.md`, `pagination-map.md`, and `synonym-ignore.md`, under their documented confirmation rules; the checking scripts remain edit-only. Everything else in a skill folder is edit-only on explicit instruction. Shared mechanics live once under `multi-skill/references/`: `verification.md` defines the two authoring packets and caller-specific authoring budgets, `verification-ledger.md` defines ingest/audit bullet evidence and audit page gates, and `verification-neutral-fixes.md` defines the re-stamp allowlist. `skill-llm-council` uses the same complementary locator/entailment role design at its skill-edit gate without producing wiki evidence. A reference belongs here only when two or more skills use it; `consistency`'s `shared_reference_integrity` check enforces that boundary.
