...
- **Skill-facing reports** (`skill-linter`, `consistency`'s `ai_writing_tells` check) use `error / warning / suggestion` — matches Anthropic's skill-authoring convention, and `error` is sharper than `critical` when the finding is "this skill is technically broken."
...
- `draft` - the note is source-supported but at least one claim on it has not been checked against its raw source. The page status carries that, so individual `*[unverified]*` markers are not needed on a draft.
- `verified` - every non-obvious claim on the page has been checked against its raw source and held. A `verified` page may still carry individual claims marked `*[unverified]*` (see Bullet Markers) — the pending delta, claims no run has yet been able to certify; everything not so marked is fact-checked.
- `needs-update` - a contradiction, stale support, source removal, coverage failure, or known gap needs attention.

`verified` is a **claim-level** guarantee, not a whole-page one: it says the page's claims check out against their sources, not that the page has been judged as a page. A page can be fully `verified` and still be two ideas wearing one title, a near-duplicate of a page written last month, or plainly LLM-voiced. Those are `audit`'s to find, and `audit` acts on them (splits, merges, rewrites, demotions) rather than stamping.
...
**Earning `verified` requires the tiered independent-refuter gate.** A wiki claim reaches `verified` only after passing an independent-refuter verification tiered by the claim's impact: obvious / own-voice / mechanical content needs no refuter, a low-stakes non-obvious claim needs one, and every other non-obvious claim needs three independent refuters that must agree — the broad default, so in practice most claims. The refuters read and reason only — spawned with read-only tools from the orchestrating agent, which applies all status itself, never nested inside another subagent — and each attempts to refute the claim against the raw; the claim is certified only on their agreement, and a split leaves the claim marked rather than certified. For a summary or generalization claim each refuter recomputes the whole set the claim ranges over, not the one cell it cites. The gate is defined once, with the full mechanics, in `.claude/skills/multi-skill/references/verification.md` (Tiered Independent-Refuter Verification); every content-authoring skill uses that one spec.

**A claim is certified where it is written, and the run that certifies may stamp.** The authoring run has the raw open and knows exactly which claims it just wrote, so it checks those claims — is the page number right, does the source really say this, is the claim attached to the source that states it — through the refuter tiers, in the same pass. Reconstructing that later costs more and confirms nothing extra. So `ingest` normally sets `verified` itself: it re-reads the whole raw and certifies every claim it writes against it. The four conditions a run must meet before stamping (coverage held, every claim on the page certified or honestly marked, the page structurally clean, nothing left that makes it misleading) and the reach rule (a run certifies only claims cited to the raw it read this pass) are in that same shared spec, under Setting Status. `synthesis`, `query`'s promotion path, and `supersede` normally finish at `draft` under the same rule, because their claims span raws the run did not each re-read.

**`audit` does not re-certify by routine.** Because claims arrive certified, `audit` is the whole-page judgement pass, not a second claim-by-claim fact-check: is this one idea or three, does it duplicate an existing page, does it contradict something, is it well connected, does it read like an LLM wrote it. It uses refuters where it has cause — a claim marked `*[unverified]*`, a page carrying claims no run ever certified (legacy or hand-edited), a claim its whole-page read gives specific reason to doubt, any claim it rewrites itself, and every claim in a `full`-mode pass — not as a blanket quorum over content already checked. The `verified_hash:` mechanism below is unchanged, and now carries more weight: the refuter gate certifies the claim against the raw, the hash certifies the checked body has not changed since, and together they are what lets a later run skip re-checking.

**Verification is claim-level for additions, not all-or-nothing for the whole page.** When a *new* non-obvious claim is added to a `verified` page without being fact-checked against the raw in the same pass — most often because the claim cites a source that run did not open — mark that claim `*[unverified]*` (Bullet Markers) rather than demoting the whole page. The page stays `verified`, the marked claim is the only pending part, and the next run that can reach its raw re-checks just it, not the whole page. This is what lets a verified page accept an incremental *addition* (a new citation on its own bullet, an added claim) without paying a full-page re-verification, and it is how a page drawing on four sources reaches `verified` one source at a time. The marker is removed by whichever run certifies the claim — a reingest of the source it cites, or `audit` — and if the claim fails instead, that run fixes it or sets the page `needs-update`.

**Changing an existing verified claim demotes the whole page to `draft` — marking it does not hold the page `verified`.** The asymmetry is mechanical, not a judgement call, and `body_hash.py` makes it exact: an *added* claim marked `*[unverified]*` was never part of `verified_hash` (a masked new line is invisible to the hash), so the page stays `verified`; but a *changed* claim was already in the hash (it was unmarked and verified), so altering its text — or marking it, which removes its line from the hashed body — moves the hash, and `lint`'s hash check resets the page to `draft` for a later run to re-verify. So an addition rides as a marked pending claim; a change re-verifies the whole page. The lone exception is a pure move: relocating a claim whose text stays byte-identical and whose meaning its new position does not change is a verification-neutral edit the moving skill re-stamps (see below), so a simple move keeps the page `verified`.

A scope-widening change to `frames:` — one where the new union admits source material the old union did not (a strict superset) — still resets the page to `draft`: it grows the unmarked body, so the whole page needs re-checking. A change that admits nothing new (a reword, or a lens the body already covers) and a pure field migration that preserves the same scope (e.g. the one-time `frame:` → `frames:` rename) do not. Mechanical metadata updates — `updated:`, `source_count`, index and log bookkeeping — do not change status. `audit` accepts a mode argument: `partial` (default) judges every page as a page and fact-checks only what it has cause to — every `*[unverified]*` claim, every page carrying claims no run certified (`draft`, `needs-update`, or a `verified` page whose hash no longer matches), any claim its whole-page read gives specific reason to doubt, and anything it rewrites itself; `full` additionally re-fact-checks every claim on every page, including certified ones. Routine runs use `partial`; the audit skill defines when a `full` pass is warranted.

A run that sets a page `verified` writes a content stamp scoped to the checked content: `verified_hash:` in the frontmatter — the SHA-256 of the page body **excluding every `*[unverified]*`-marked claim**. The exclusion is what makes the claim-level model work:
...
- A non-`verified` page (`draft`/`needs-update`) carries no `verified_hash:`; any operation that demotes a page strips it, and `lint` strips a `verified_hash:` it finds on a non-`verified` page. The hash is re-added only by the next run that certifies the page's claims.

The hash covers the body (minus marked claims) only, so mechanical frontmatter updates do not trip it. Every skill that reads or writes the stamp — `lint`, `ingest`, `audit`, `supersede`, `forget` — computes it with `.claude/skills/multi-skill/scripts/body_hash.py`, which masks `*[unverified]*` claims before hashing. Never hand-compute or hand-copy a hash.
...
- unwrapping an existing wikilink to plain text inside a reciprocity callout (`Contradictions` / `Tensions`) where the linked page is an *incidental* (non-party) mention, not a party to the disagreement, and the rendered display stays byte-identical to the plain text it becomes — the symmetric inverse of the wrap above, and the schema-prescribed cure for a `missing_reciprocal_contradiction` over-count (the check reads *every* callout wikilink as a party, so an incidental link asserts a disagreement that does not exist). The party-versus-incidental call is a judgement the check cannot make, so `audit` unwraps only a mention it confirms is a non-party, records the now-unlinked occurrence in the check's verified-ignore list (`unlinked-mention-ignore.md`) with the reciprocity-callout reason, and authors the reciprocal instead of unwrapping whenever the call is uncertain — erasing a genuine contradiction relationship is worse than completing one;
...
- a source-page locator-anchor *relocation* that repositions an **already-present** structural anchor relative to the `#page=N` deep-link without changing which section/figure or page it names (`sec. 3.2, [[…#page=9|p. 9]]` → `[[…#page=9|sec. 3.2, p. 9]]`; lint's `source_locator_incomplete`) — the reader reads the same locator, so no claim moves. **Hard exclusion:** *adding* an anchor to a page-only locator, or *changing* which section/figure/page a locator names (e.g. relabelling abstract-drawn content `sec. 1`), is **not** on this allowlist. That asserts a new fact about where the cited content sits, which only the raw can settle. So it takes one of two routes, never a bare re-stamp: the run **certifies it against the raw** — opening the raw at the cited page and settling the anchor by the nearest heading above the cited text, through the refuter tiers, which is question 1 of the claim check — and then stamps on that certification; or, where the run cannot or does not check it, the locator's claim is marked `*[unverified]*` and the page demotes to `draft` for a later run to settle. What stays forbidden is the middle path that produced the abstract→`sec. 1` mislabels: an agent *inferring* an anchor from the page number or the wiki page and stamping its own inference. The guard is not "who wrote it" but "was the raw opened at that page" — a certified anchor is verified wherever it was written; an inferred one is not, even in an `audit` run.
...
The allowlist is partitioned by which skill applies each edit. `lint` owns the four format fixes (it never edits callout body prose, so de-hyphenation and spelling are not lint's to apply). `audit` owns de-hyphenation (both the always-open and the slug-derived noun-only mappings), spelling normalization, the `unlinked_page_mention` wikilink wrap, and the incidental reciprocity-callout link unwrap. A stale-path repair is applied by whichever skill runs the rename cascade (`forget` / `supersede` / `ingest`); a content-identical claim relocation, by whichever skill performs the move (`supersede`, `ingest`, or `audit`). Each re-stamps under the same rule.
...
**Everything else demotes — the backstop is unchanged.** Any edit whose claim-neutrality is not machine-provable resets the page to `draft` (stripping the stale `verified_hash:`), exactly as before: a new or changed non-obvious claim, a reworded bullet even when "it means the same thing", a citation whose source-page target or locator changes, an embed whose target image changes, an edited number or quote. A prose reword you *judge* faithful is not a string transform you can *prove* claim-neutral — it demotes and a later run re-verifies. The allowlist is deliberately small and string-transform-shaped precisely so that "is this edit claim-neutral?" never becomes a judgement call that could let a real claim change ride.

So the rule for editing a `verified` page is: **certify each non-obvious claim you add or change against the raw, in the same pass** — then the page keeps its status and you re-stamp; **mark what you cannot certify `*[unverified]*`** — an addition you mark rides as the pending delta and the page stays `verified`, while a *change* you mark still demotes it, because that claim's line was in `verified_hash` and altering or masking it moves the hash. If you change verified (unmarked) content and neither certify it, mark it as an addition, nor make an allowlisted verification-neutral edit (a pure move included), `lint`'s hash check demotes the whole page to `draft` and a later run re-verifies it. Three re-stamps are legitimate: (1) the certifying run's own, when it has checked the claims it wrote or changed against the raw through the refuter tiers and the four Setting Status conditions hold; (2) `audit`'s, when it fact-checks a page against the raw during a run — clearing `*[unverified]*` markers or fixing claims — for the pages it checked; and (3) the verification-neutral re-stamp above, when a skill applies an allowlisted determinate edit. Never re-stamp `verified_hash` by hand after any *other* unmarked body edit; let the hash-mismatch reset fire. Outside these three cases, `verified_hash` is earned only by a fact-check against the raw source.

## Bullet markers

Most bullets have no suffix. Use markers sparingly:

- `*\[unverified\]*` for a non-obvious claim awaiting a raw fact-check. Written by the run that wrote the claim but could not certify it — most often because the claim cites a source that run did not open, sometimes because its refuters split or the cited region was unreadable — so the page stays `verified` while only that claim is the pending delta (see Page Status). Marking a *changed* existing claim does not keep the page `verified` — a change moves the hash and demotes the page to `draft` — so the marker is for additions, not changes. It is never a way to skip checking a claim cited to the raw the run had open; that claim is the run's to certify or fix. Whichever run later certifies the claim removes the marker. Distinct from `*\[tentative\]*`: `*\[unverified\]*` is a process state (not yet checked, removed on check); `*\[tentative\]*` is an epistemic judgement (support is weak) that persists even after verification. A claim can carry both. Not used on `draft` pages, where the page status already means something on the page is unchecked.
...
- `ingest` - process one raw source into a source page and propose only the concept/entity pages it genuinely supports, then certify every claim it wrote against the raw it has open — the page number, what the source says, which source the claim is attached to — through independent refuters, and set the page's status on that. Auto-detects mode: creates the page first-time, or reingests in place for a stated reason if it already exists. Deep mode produces a paper-grade source page for a load-bearing source. Optional frames scope the source page and accumulate across reingests. For a book, asks up front which chapters or page range to read.
...
- `audit` - whole-page judgement: one-idea clarity, duplicates, contradictions, connectedness, and LLM voice, on the finished page — the questions that cannot be asked claim by claim. Uses refuters on cause (an unverified marker, an uncertified page, a claim it doubts, a claim it rewrites), not as a blanket re-check of certified content. Satisfies its own preconditions by running `consistency` and `lint` when their reports are stale or not clean, and re-runs `lint` over the pages it changed.
...
- `cleanup` - three jobs: check whether each memory-file entry has already graduated into its permanent home (MEMORY.md, CLAUDE.md, or a skill) and clear only the absorbed ones — it never writes a rule into a home itself, so an un-absorbed entry is reported with its home and the text to add, and left resident; prune unneeded `2-outputs/` files (junk, superseded check reports, subject-orphaned reports, aged artifacts), reporting rather than proposing anything it cannot classify; and strike finished work from `hot.md`'s Open threads and Watchlist, acting on `lint`'s detect-only `hot_thread_spent` findings. Every deletion is gated on the user's approval — per file, or as one multiSelect batch where every file in it is verified git-recoverable — and recorded in the run's log entry with the commit that last held it. A hot-thread prune is gated one entry per call whatever its git state, like a memory entry: each entry's spent-versus-live split is its own judgement, so a batch would collapse distinct calls into one tick.
...
- Every ingest, reingest, and deep ingest fully re-reads the raw source. The reason, frames, and depth control what gets written, never how much gets read — there is no targeted partial re-read. **The one exception is a book** (a raw under `0-raw/books/`, written as `type: book`): a book is large and usually only a single chapter is wanted, so `ingest` asks up front for the chapter or page range and reads only that range. The chosen range is then the read unit and is read *in full* — the exception scopes what counts as "the whole document" down to the range, it is not licence to skim. The book's source page is `1-wiki/sources/{stem}.md` (the stem matching the raw, as for any source), scoped to that range, with the chapter or section named in `title:`. A long survey or paper is still read cover to cover — only a book is scoped by an up-front range prompt.
- Every operation that authors or changes durable `1-wiki/` content runs the shared verification (`.claude/skills/multi-skill/references/verification.md`) before finalizing: the **claim check** over exactly the claims it wrote — the page number, what the source says, which source the claim is attached to — certified through the tiered independent-refuter gate, and the light **page self-check** over the pages it touched. It then sets each page's status by that spec's Setting Status rule: `verified` when the four conditions hold, `draft` when a claim on the page is neither certified nor honestly marked, `needs-update` when the run leaves an unresolved problem. This binds `ingest` (Step 8, every mode — new or existing source, normal or deep), `query`'s page-authoring path, `synthesis` (Step 8), and `supersede` (Step 7, over the content it newly authored or changed); `ingest` normally reaches `verified`, the others normally finish at `draft` because their claims span raws they did not each re-read. The two layers are now division of labour rather than repetition: the authoring run owns the claims (checked where they are written, while the raw is open), and `audit` owns the page (the judgements that need the finished page in view). `audit` therefore runs no claim check on itself — it certifies what it rewrites, and uses refuters elsewhere on cause. Read-only and output-only skills (`brief`, `compare`, `reflect`, a plain `query` with no promotion) author no durable wiki content and run neither check; `forget` removes rather than authors.
...
- Ask before adding or changing substantive wiki content. Mechanical updates such as `updated:`, `source_count`, index, and log entries do not need approval. The exception is `audit`: having read the page as a whole, and having confirmed against the raw whatever its judgement turns on, it autonomously applies status changes and content fixes, preserving the prior version — see the audit skill. This applies to synthesis pages too: `audit` judges a synthesis page and carries out the resulting fixes autonomously — correcting a distorted or over-generalized `Answer` claim, adding a missing citation, marking a claim `*[unverified]*`, preserving a real disagreement in `Tensions`, and setting `verified` / `needs-update` — exactly as it does any other page, preserving the prior version. A synthesis page needs that pass more than most: it normally arrives `draft` because its claims span raws the synthesis run did not each re-read, and it is the page type most prone to sounding more universal than its sources support. The `## Synthesis Pages` approval gate ("Do not create, update, or promote a synthesis without user approval") binds the `synthesis` skill and user-initiated changes; it does not fence off `audit`, the standing autonomy exception — audit's authored-fixes procedure, including a fact-check-driven split or merge, applies to synthesis pages as to any other, preserving the prior view.
...
`audit` emits the same kind of field back, so the convention runs both ways rather than only into audit: its report carries `result: settled | interim | blocked` plus `pages_pending:`. `settled` means every page audit's scope reached ended `verified`, or `needs-update` with a `needs_update_reason:` naming a blocker no further pass can clear, with nothing on its worklist unapplied; `interim` means a further pass still has work (a page the staged valve left `draft`, a claim marked pending a raw a later run will reach), and `pages_pending:` counts those pages; `blocked` means a precondition stopped the run, or the bounded-rounds valve was reached without approval. A successor audit pass, and `cleanup`'s superseded-report classification, read that field rather than re-deriving the run's terminal state from prose.
...
3. **Per-skill and multi-skill memory — recent append-only corrections.** Two files:
   ...
   - `.claude/skills/multi-skill/multi-skill-memory.md` — corrections that apply across more than one skill. Read by every skill, write and read-only alike, at the start of every operation.
...
Recent activity is a rolling cache of the five newest entries: `lint` trims it to five, and because every entry is also recorded in `1-wiki/log.md` (the permanent, complete record of every operation), trimming the cache loses nothing. That five-entry count is the section's retention policy — owned here so one rule governs it; the whole-file soft cap of 200 lines (stated in hot.md's header) is a separate, consistent bound. Open threads and Watchlist are not caches — they hold unique orientation that is not duplicated in `log.md`, so they are never trimmed on a count. They are pruned on completion instead: both sections carry only outstanding work, so a finished sub-item is struck from its entry and the whole entry is removed once nothing live remains (a target page that no longer exists is one case of this). Completion is read off the wiki's current state rather than remembered — an entry asking for the `*[unverified]*` markers on a page to be cleared is spent once that page carries none, one calling a page draft or awaiting `/audit` is spent once the page is `verified`, and a deferred sweep is spent once `lint` no longer reports that check. Finished work is not reworded to keep its place in the section: it is already recorded in `log.md`, so it leaves `hot.md` entirely rather than being restated in the past tense. `lint`'s `hot_thread_spent` detects a spent entry but never prunes it — an entry usually mixes finished and live sub-items, so which part to strike is a judgement, and `cleanup`'s hot-thread job is what acts on the finding.
...
Log entries are historical records, not a view of the current state: each says what one operation did at the time it ran. They are never edited to match how the wiki looks afterwards — an entry recording that a run added an `*[unverified]*` marker, left a page `draft`, or wrote a since-deleted report stays exactly as written once that marker is cleared, that page verified, or that report removed. Rewriting them would falsify the operation history, which is also what makes `hot.md` safe to prune: pruning a finished thread loses nothing precisely because `log.md` still holds the record of the work. This is the same principle Stay in your lane already applies to paths, where historical `log.md` entries recording a past path are left alone.

## Stay in your lane

- `0-raw/` is hard read-only.
- `a-archive/` is soft read-only — user-curated reference material. Edit only on explicit instruction.
- `.claude/skills/` is soft read-only, with these exceptions: `.claude/skills/multi-skill/multi-skill-memory.md` (cross-skill corrections) and each `.claude/skills/<skill>/<skill>-memory.md` file (skill-specific corrections) are agent-writable under workflow discipline (append new entries on user-confirmed corrections; don't silently rewrite existing ones), and four shared data files are agent-writable curated data — all data loaded at runtime, not logic, so the checks' code in `check_wiki.py` stays edit-only. Three feed heuristic checks that need a human-confirmed escape valve: `.claude/skills/multi-skill/hyphenation-lists.md` (the four lists behind `hyphenated_open_compound_noun`), `.claude/skills/multi-skill/unlinked-mention-ignore.md` (the verified-ignore list behind `unlinked_page_mention`, recording an occurrence confirmed to be generic wording rather than a genuine reference), and `.claude/skills/multi-skill/alias-detect-exempt.md` (the per-page detect-exemption list behind the same check, recording a display form that stays in its page's `aliases:` for Obsidian search and `[[` autocomplete but leaves the check's matching vocabulary). The two `unlinked_page_mention` files answer different questions and are not interchangeable: the ignore list records a per-occurrence judgement that *this* mention is generic, while an exemption retires *one form of one page* from detection altogether. Reach for the exemption when a form is so common that recording occurrences has stopped paying for itself — many suppressions accumulating on one form is a detector problem, not a data problem — and prefer exempting one form over dropping the alias, which would cost the page its retrieval handle as well as its detection. The exemption is scoped to the one page named, so an identical form on any other page keeps detecting normally, and it never exempts a page from its own forms. Its cost is that the form's genuine unlinked references stop being flagged too, so every entry carries the measured genuine-versus-generic count that justified it; an entry that exempts nothing is reported as `stale_alias_exempt`. `audit` grows all three autonomously, each addition sub-agent-verified (audit's Step 7). The fourth, `.claude/skills/multi-skill/pagination-map.md`, is the per-raw record of what each physical page prints (behind `locator_page_mismatch` and the anchor-only locator exemption in `citation_locator_incomplete` / `source_locator_incomplete`); it is registered on ingest, not grown by audit — `scripts/pagination_map.py` proposes a map from the PDF's footers and a human confirms each line against a rendered footer before it lands, since a wrong `none` would license stripping a correct printed page from a citation. Analogously, `.claude/skills/multi-skill/synonym-ignore.md` is agent-writable curated data — the per-skill confirmed-distinct terminology allow-list behind `check_synonyms.py`'s `terminology_candidate` suppression; `skill-linter` grows it autonomously when a run confirms a flagged synonym pair names genuinely different things in a skill (capturing the keep-vs-drop judgement it already makes), and removing an entry re-surfaces that candidate. The rest of each skill folder (SKILL.md, references, scripts) is edit-only on explicit instruction. The `.claude/skills/multi-skill/` folder holds cross-skill shared materials: the `multi-skill-memory.md` journal (agent-writable, above) and `references/` — `dependent-cascade.md`, `inbound-reference-discovery.md`, and `quarantine-path-convention.md`, the common mechanics cited by the page-mutating skills (`forget`, `supersede`, `ingest`), plus `relationship-sweep.md`; `verification.md`, the verification spec run by `ingest` (Step 8), `query` (its page-authoring path), `synthesis` (Step 8), and `supersede` (Step 7); `verification-neutral-fixes.md`, the verification-neutral edit allowlist cited by `lint` and `audit`; `self-report.md`, the canonical statement of the per-run self-report every skill ends with; and the skill-authoring pair `skill-authoring-checklist.md` and `skill-authoring-checks.md`, cited by `skill-linter` and `skill-llm-council` — so the logic lives in one place rather than drifting across copies. A reference belongs in `multi-skill/references/` only when it is genuinely shared (cited by two or more skills); a file used by one skill belongs in that skill's own `references/`, and the spec must exist as a single copy here, never duplicated into a skill folder. `consistency`'s `shared_reference_integrity` check enforces both. Those reference files, like SKILL.md and per-skill references, are edit-only on explicit instruction.
